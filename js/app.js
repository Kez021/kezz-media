...
async function loadManualBadges(uid, containerId, isOwner){
  ...
  try{
    ...
    wrap.innerHTML = myBadges.map(function(b){
      ...
      return '<div title="'+esc(title)+'" style="width:34px;height:34px;border-radius:50%;background:'+(def.color||'#e2688a')+';display:flex;align-items:center;justify-content:center;font-size:16px;border:1.5px solid var(--card);">'+(def.icon||'🏅')+'</div>';
    }).join('');
...
function renderPostGrid(){
  ...
  var myPosts=posts.filter(p=>p.uid===me.uid && !p.isStatus && !p.isPoll);
...
function handleFiles(inp){
  ...
  files.forEach(f=>{
    ...
    if(isVideo){
      // Videos: keep the raw File for upload, use an object URL for the preview only (no base64 — too heavy)
      const item={type:'video', file:f, previewUrl:URL.createObjectURL(f)};
      ...
      const r=new FileReader();
      r.onload=e=>{
        const item={type:'image', file:f, dataUrl:e.target.result, previewUrl:e.target.result};
        uploadMediaItems.push(item);
        renderUploadPreviews();
        document.getElementById('uploadZone').style.display='none';
      };
...
function removeUploadImg(i){
  ...
  if(item && item.type==='video' && item.previewUrl) URL.revokeObjectURL(item.previewUrl);
    ...
    for(let i=0;i<uploadMediaItems.length;i++){
      ...
      try{
        if(item.type==='video'){
          ...
          const url=await uploadToStorage(item.dataUrl, `posts/${uid}/${Date.now()}_${i}.jpg`);
    ...
    var html =
      '<div class="profile-card">'
        +'<div class="opu-banner" style="background:'+bannerBg+';">'
          ...
          +'</button>'
          +'<div id="opuRecentVisitors" style="position:absolute;top:10px;right:10px;z-index:3;"></div>'
        +'</div>'
        +'<div class="opu-top">'
          ...
          +(Array.isArray(prof.pinnedAchievements)&&prof.pinnedAchievements.length
            ? '<div style="display:flex;gap:6px;margin-bottom:8px;">'+prof.pinnedAchievements.map(function(id){
                ...
                if(!def) return '';
                return '<div title="'+esc(def.name)+'" style="width:34px;height:34px;border-radius:50%;background:var(--bg2);border:1.5px solid var(--pink);display:flex;align-items:center;justify-content:center;font-size:17px;">'+def.icon+'</div>';
              }).join('')+'</div>'
            : '')
...
async function publishStory(){
  ...
  var storyRef=await db.collection('stories').add(storyData);
...
var ACHIEVEMENT_DEFS = [
  // Beginner
  {id:'first_login',   cat:'Beginner', name:'First Login',    icon:'👋', check:function(s){return s.logins>=1;}},
  {id:'first_post',    cat:'Beginner', name:'First Post',     icon:'📝', check:function(s){return s.postsCount>=1;}},
  {id:'first_story',   cat:'Beginner', name:'First Story',    icon:'📸', check:function(s){return s.storiesCount>=1;}},
  {id:'first_comment', cat:'Beginner', name:'First Comment',  icon:'💬', check:function(s){return s.commentsMade>=1;}},
  {id:'first_like',    cat:'Beginner', name:'First Like',     icon:'❤️', check:function(s){return s.likesGiven>=1;}},
  {id:'first_friend',  cat:'Beginner', name:'First Friend',   icon:'🤝', check:function(s){return s.followingCount>=1;}},
  {id:'complete_profile',cat:'Beginner',name:'Complete Profile',icon:'✅', check:function(s){return s.profileComplete;}},
  // Creator
  {id:'posts_10',   cat:'Creator', name:'10 Posts',   icon:'🖼️', progress:'postsCount',   target:10,   check:function(s){return s.postsCount>=10;}},
  {id:'stories_25', cat:'Creator', name:'25 Stories',  icon:'🎞️', progress:'storiesCount', target:25,   check:function(s){return s.storiesCount>=25;}},
  {id:'likes_100',  cat:'Creator', name:'100 Likes Received',  icon:'💗', progress:'likesReceived', target:100,  check:function(s){return s.likesReceived>=100;}},
  {id:'likes_500',  cat:'Creator', name:'500 Likes Received',  icon:'💖', progress:'likesReceived', target:500,  check:function(s){return s.likesReceived>=500;}},
  {id:'likes_1000', cat:'Creator', name:'Popular Creator (1000 Likes)', icon:'🌟', progress:'likesReceived', target:1000, check:function(s){return s.likesReceived>=1000;}},
  // Community
  {id:'comments_100', cat:'Community', name:'100 Comments',   icon:'🗨️', progress:'commentsMade', target:100, check:function(s){return s.commentsMade>=100;}},
  {id:'replies_50',   cat:'Community', name:'50 Replies',     icon:'↩️', progress:'repliesMade',  target:50,  check:function(s){return s.repliesMade>=50;}},
  {id:'reactions_500',cat:'Community', name:'Friendly User (500 Reactions)', icon:'🎉', progress:'likesGiven', target:500, check:function(s){return s.likesGiven>=500;}},
  // Explorer
  {id:'visit_100', cat:'Explorer', name:'Visit 100 Profiles', icon:'🧭', progress:'profilesVisited', target:100, check:function(s){return s.profilesVisited>=100;}},
  {id:'visit_500', cat:'Explorer', name:'Visit 500 Profiles', icon:'🗺️', progress:'profilesVisited', target:500, check:function(s){return s.profilesVisited>=500;}},
  // Streak
  {id:'streak_3',   cat:'Streak', name:'3-Day Streak',   icon:'🔥', progress:'loginStreak', target:3,   check:function(s){return s.loginStreak>=3;}},
  {id:'streak_7',   cat:'Streak', name:'7-Day Streak',   icon:'🔥', progress:'loginStreak', target:7,   check:function(s){return s.loginStreak>=7;}},
  {id:'streak_30',  cat:'Streak', name:'30-Day Streak',  icon:'🔥', progress:'loginStreak', target:30,  check:function(s){return s.loginStreak>=30;}},
  {id:'streak_100', cat:'Streak', name:'100-Day Streak', icon:'🔥', progress:'loginStreak', target:100, check:function(s){return s.loginStreak>=100;}},
  {id:'streak_365', cat:'Streak', name:'365-Day Streak', icon:'🔥', progress:'loginStreak', target:365, check:function(s){return s.loginStreak>=365;}},
  // Special
  {id:'verified_user', cat:'Special', name:'Verified User', icon:'✔️', check:function(s){return !!s.verified;}},
  {id:'founding_member',cat:'Special', name:'Founding Member', icon:'🏛️', check:function(s){return !!s.foundingMember;}}
];
...
async function checkAchievements(){
  ...
  for(var i=0;i<newlyUnlocked.length;i++){
    ...
    toast(def.icon+' Achievement unlocked: '+def.name+'!');
  }
}
  ...
  var html = Object.keys(cats).map(function(cat){
    var items = cats[cat].map(function(def){
      ...
      var pinBtn = (isMe && unlocked)
        ...
        : (unlocked?'<div style="font-size:10.5px;color:var(--pink);font-weight:700;flex-shrink:0;">UNLOCKED</div>':'<div style="font-size:14px;flex-shrink:0;">🔒</div>');
      return '<div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid var(--border);opacity:'+(unlocked?'1':'.5')+';">'
        +'<div style="font-size:22px;flex-shrink:0;">'+def.icon+'</div>'
...
async function openProfileInfoPanel(uid){
  ...
  var joined = 'Unknown';
  if(prof.createdAt){
    var ms = prof.createdAt.toMillis ? prof.createdAt.toMillis() : (prof.createdAt.seconds ? prof.createdAt.seconds*1000 : null);
...
async function loadMutualFriends(uid){
  ...
  try{
    ...
    var names = mutualUids.slice(0,3).map(function(u){
      var prof = allUsers[u];
      return prof ? (prof.name||prof.handle||'Someone') : 'Someone';
    });
    var extra = mutualUids.length>3 ? ' + '+(mutualUids.length-3)+' more' : '';
    wrap.innerHTML = '<div style="font-size:12px;color:var(--text3);">👥 Followed by <strong style="color:var(--text2);">'+names.map(esc).join(', ')+'</strong>'+extra+'</div>';
