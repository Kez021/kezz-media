...
// ── FIREBASE INIT ─────────────────────────────────────
// firebaseConfig now lives in config/firebase.js (loaded before this file)
...
const communityPostsCol = (communityId) => db.collection('communities').doc(communityId).collection('posts');
// Returns the correct Firestore collection for a post — the main /posts collection for regular
// posts, or a community's own posts subcollection for posts tagged with _community. This lets
// community posts reuse buildCard/doLike/addComment/etc. completely unmodified, since the only
// thing that differs is which collection the write ultimately lands in.
function _postCollFor(post){
  return (post && post._community) ? communityPostsCol(post._community) : postsCol();
}
...
async function uploadToStorage(base64, path) {
  ...
  try {
    ...
    if(path) form.append('folder', 'kez-media/'+path.split('/').slice(0,-1).join('/'));
...
// Upload an image directly from its raw File object — no base64 involved anywhere.
// Reading many images as base64 upfront (the old approach) held every photo fully decoded in
// memory at once; with 20+ high-res phone photos that's easily 100+ MB, enough to freeze or
// silently kill the tab on mobile before the post ever reaches Firestore. This uploads straight
// from the File/Blob handle instead, which stays cheap regardless of how many photos are picked.
...
// Firestore rejects `undefined` anywhere in a document (even nested) and throws — silently, with no
// UI feedback, right at the .set()/.add() call. `me` has picked up many optional fields over time
// (achievements, streaks, privacy settings, etc.) that can be `undefined` for users who haven't
// triggered them yet. This strips undefined → the write always succeeds instead of vanishing.
function _stripUndefined(obj){
  if(obj===null || typeof obj!=='object') return obj;
  if(Array.isArray(obj)) return obj.map(_stripUndefined);
  // Never touch Firestore sentinel values (serverTimestamp(), increment(), etc.) — recursing into
  // them via Object.keys() would strip their special internal type and break the write.
  if(firebase.firestore.FieldValue && obj instanceof firebase.firestore.FieldValue) return obj;
  if(obj instanceof firebase.firestore.Timestamp) return obj;
  var clean = {};
  Object.keys(obj).forEach(function(k){
    var v = obj[k];
    if(v===undefined) return; // drop it entirely rather than writing `undefined`
    clean[k] = (v && typeof v==='object') ? _stripUndefined(v) : v;
  });
  return clean;
}
...
async function savePostToFirestore(p) {
  try{
    await _postCollFor(p).doc(String(p.id)).set(_stripUndefined({
      ...p,
      createdAt: firebase.firestore.FieldValue.serverTimestamp()
    }));
  }catch(e){
    console.error('savePostToFirestore failed:', e);
    toast('Post upload failed to save — please try again (your photos/videos are safe, just the post itself didn\'t save).');
    throw e;
  }
}
async function updatePostField(id, updates) {
  try {
    ...
    await _postCollFor(p).doc(String(id)).update(_stripUndefined(updates));
  } catch(e) {
    console.error('updatePostField failed ['+id+']', e.code||e.message);
    throw e;
  }
}
async function deletePostFromFirestore(id) {
  var p = posts.find(function(x){return String(x.id)===String(id);});
  await _postCollFor(p).doc(String(id)).delete();
}
...
let uploadImages = []; // legacy array of base64 strings (kept for other callers, e.g. poll photos)
let uploadMediaItems = []; // [{type:'image'|'video', file:File, previewUrl:string, dataUrl?:string}] — preserves upload order for mixed posts
...
async function sendNotification(toUid, type, fromUser, extra, postId){
...
function showNotifPopup(nid, n){
  ...
  item.innerHTML =
    ...
    +'<div class="notif-popup-body">'
      +'<strong>'+esc(n.fromName||n.fromHandle||'Someone')+'</strong>'+verifiedBadgeHTML(_liveUserFor(n.fromUid,{verified:n.fromVerified}),12)+' '
  ...
  item.addEventListener('click', function(e){
    ...
    dismissPopup(item);
    if(n.postId && (n.type==='like'||n.type==='comment'||n.type==='repost'||n.type==='share'||n.type==='mention'||n.type==='vote')){
      goTo('home');
      setTimeout(function(){
...
function renderNotifs(docs){
  ...
  const icons={like:'❤️',comment:'💬',follow:'👤',message:'✉️',mention:'@',repost:'🔁',share:'📤',vote:'🗳️'};
  const texts={like:'liked your post',comment:'commented on your post',follow:'started following you',message:'sent you a message',mention:'mentioned you',repost:'reposted your post',share:'shared your post',vote:'voted on your poll'};
  docs.forEach(d=>{
    const n=d.data(); const nid=d.id;
    const div=document.createElement('div');
    div.className='notif-item'+(n.read?'':' unread');
    div.style.cursor='pointer';
    // For vote type: n.extra is already the full phrase e.g. 'voted for "oppa reggb" on your poll'
    // For other types: append extra as a preview snippet
    var bodyText;
    if(n.type==='vote' && n.extra){
      bodyText = n.extra; // full descriptive text already set by sendNotification
    } else {
      bodyText = (texts[n.type]||n.type) + (n.extra?' — <em>'+esc(n.extra.slice(0,40))+'</em>':'');
    }
    div.innerHTML=
      ...
      +'<div class="notif-text"><strong>'+esc(n.fromName||n.fromHandle||'Someone')+'</strong>'+verifiedBadgeHTML(_liveUserFor(n.fromUid,{verified:n.fromVerified}),12)+' '+bodyText+'</div>'
      ...
      if(n.postId && (n.type==='like'||n.type==='comment'||n.type==='repost'||n.type==='share'||n.type==='mention'||n.type==='vote')){
        ...
        setTimeout(function(){
          ...
          if(card){
            ...
            card.style.outline='2.5px solid var(--pink)';
            card.style.boxShadow='0 0 0 4px rgba(226,104,138,.18)';
            setTimeout(function(){card.style.outline='';card.style.boxShadow='';},2500);
          } else {
            openLightbox(n.postId);
          }
        },350);
      } else if(n.type==='follow' && n.fromUid){
        viewProfile(n.fromUid);
...
async function init(){
  ...
  _updateLoginStreak();
  loadMyAchievements();
  archiveMyExpiredStories(); // auto-move expired stories to storyArchive
...
function applyUserProfile(){
  ...
  const eip=document.getElementById('editIsPrivate');if(eip)eip.checked=!!me.isPrivate;
}
...
function saveProfile(){
  ...
  const priv=document.getElementById('editIsPrivate');
  ...
  if(priv) me.isPrivate = !!priv.checked;
...
// ── RICH LINK PREVIEWS ─────────────────────────────────
// Client-side JS can't read another site's HTML directly (CORS blocks it for almost every real
// website). Using microlink.io's free metadata API instead — it fetches the target page server-side
// and returns clean JSON (title/description/image/domain) with proper CORS headers so we can call it
// straight from the browser. No backend/Cloud Function needed.
var _linkPreviewCache = {}; // url -> {title,description,image,domain} — avoids repeat API calls this session
var URL_REGEX = /(https?:\/\/[^\s<]+)/i;

function extractFirstUrl(text){
  if(!text) return null;
  var m = text.match(URL_REGEX);
  return m ? m[0].replace(/[.,;:!?)\]]+$/,'') : null; // trim trailing punctuation caught by the regex
}

// Note: URL tokenization + linking happens inline inside linkifyCaption() below (needed to avoid
// hashtag/mention regexes corrupting anchor tag markup — see comment there for why).

function isPreviewableUrl(url){
  try{
    var u = new URL(url);
    if(u.protocol!=='http:' && u.protocol!=='https:') return false; // block javascript:, data:, etc.
    // Skip known-unsupported or risky link shorteners/local addresses — keeps this conservative
    if(/^(localhost|127\.|0\.0\.0\.0|192\.168\.|10\.)/.test(u.hostname)) return false;
    return true;
...
async function fetchLinkPreview(url){
  ...
  try{
    var res = await fetch('https://api.microlink.io/?url='+encodeURIComponent(url)+'&meta=false');
...
function buildLinkPreviewHTML(preview){
  ...
  return '<a href="'+esc(preview.url)+'" target="_blank" rel="noopener noreferrer" style="display:flex;text-decoration:none;color:inherit;border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-top:8px;background:var(--bg2);">'
    + (preview.image ? '<div style="width:88px;height:88px;flex-shrink:0;background:var(--bg3);"><img src="'+esc(preview.image)+'" style="width:100%;height:100%;object-fit:cover;display:block;" onerror="this.parentElement.style.display=\'none\';"></div>' : '')
    + '<div style="padding:10px 12px;min-width:0;overflow:hidden;">'
      + '<div style="font-size:10.5px;color:var(--text3);text-transform:uppercase;letter-spacing:.03em;">'+esc(preview.domain)+'</div>'
      + '<div style="font-size:13px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2px;">'+esc(preview.title)+'</div>'
      + (preview.description?'<div style="font-size:11.5px;color:var(--text3);margin-top:2px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">'+esc(preview.description)+'</div>':'')
    + '</div></a>';
}

// Renders a link preview into a container element for a given caption/message text, fetching + caching
// as needed, and (if a Firestore ref + field name are given) persists the result so it's never re-fetched.
async function renderLinkPreviewInto(containerEl, text, docRef, fieldName){
  if(!containerEl) return;
  var url = extractFirstUrl(text);
  if(!url){ containerEl.innerHTML=''; return; }
  var preview = await fetchLinkPreview(url);
  if(!preview){ containerEl.innerHTML=''; return; }
  containerEl.innerHTML = buildLinkPreviewHTML(preview);
  if(docRef && fieldName){
    docRef.update(_stripUndefined({ [fieldName]: preview })).catch(function(){});
  }
}

function linkifyCaption(txt){
  if(!txt) return '';
  // Tokenize into URL / plain-text segments FIRST, so the hashtag/mention regexes below never run
  // against text that's already inside an <a href="..."> tag — URLs frequently contain # characters
  // (fragments) which would otherwise get corrupted into a hashtag span mid-attribute.
  var seenFirst = false;
  var parts = txt.split(/(https?:\/\/[^\s<]+)/gi);
  var html = parts.map(function(part){
    if(/^https?:\/\//i.test(part)){
      var url = part.replace(/[.,;:!?)\]]+$/,'');
      var trailing = part.slice(url.length);
      return '<a href="'+esc(url)+'" target="_blank" rel="noopener noreferrer" style="color:var(--pink);text-decoration:underline;word-break:break-all;">'+esc(url)+'</a>'+esc(trailing);
    }
...
var _exploreTab = 'posts';
function setExploreTab(tab){
  _exploreTab = tab;
  document.querySelectorAll('#exploreTabs [data-etab]').forEach(function(b){
    b.classList.toggle('on', b.dataset.etab===tab);
  });
  renderExploreFeed();
}
function renderExploreFeed(){
  const ef=document.getElementById('exploreFeed');
  ef.innerHTML='';
  var publicPosts = posts.filter(function(p){
    if(p._community) return false; // community posts stay inside their community, never in Explore
    // Respect private accounts — don't surface their posts to non-followers in a public discovery feed
    var owner = allUsers[p.uid];
    if(owner && owner.isPrivate && p.uid!==me.uid && !(me.following||[]).includes(p.uid)) return false;
    return true;
  });

  if(_exploreTab==='tags'){
    var tagCounts = {};
    publicPosts.forEach(function(p){
      if(!p.caption) return;
      var matches = p.caption.match(/#(\w+)/g) || [];
      matches.forEach(function(m){
        var tag = m.slice(1).toLowerCase();
        tagCounts[tag] = (tagCounts[tag]||0)+1;
      });
    });
    var tags = Object.keys(tagCounts).sort(function(a,b){return tagCounts[b]-tagCounts[a];});
    if(!tags.length){ ef.innerHTML='<div class="empty">No hashtags yet — add a #tag to a post caption to start one.</div>'; return; }
    var wrap = document.createElement('div');
    wrap.style.cssText='display:flex;flex-direction:column;gap:2px;padding:8px 4px;';
    tags.forEach(function(tag){
      var row=document.createElement('div');
      row.style.cssText='display:flex;align-items:center;justify-content:space-between;padding:12px 12px;border-radius:12px;cursor:pointer;';
      row.onmouseover=function(){row.style.background='var(--bg2)';};
      row.onmouseout=function(){row.style.background='none';};
      row.innerHTML='<div style="font-weight:700;font-size:14px;color:var(--pink);">#'+esc(tag)+'</div><div style="font-size:12px;color:var(--text3);">'+tagCounts[tag]+' post'+(tagCounts[tag]!==1?'s':'')+'</div>';
      row.onclick=function(){showHashtag(tag);};
  ...
  if(_exploreTab==='communities'){
    ef.innerHTML='';
    ef.appendChild(buildCommunityCard());
    return;
  }

  var filtered = publicPosts;
  if(_exploreTab==='videos'){
    filtered = publicPosts.filter(function(p){ return (p.media&&p.media.some(function(m){return m.type==='video';})) || (p.isVideo&&p.videoUrl); });
  } else if(_exploreTab==='photos'){
    filtered = publicPosts.filter(function(p){
      if(p.isStatus||p.isPoll) return false;
      var hasImg = (p.images&&p.images.length) || p.image || (p.media&&p.media.some(function(m){return m.type==='image';}));
      var hasVid = (p.media&&p.media.some(function(m){return m.type==='video';})) || p.isVideo;
      return hasImg && !hasVid;
    });
  } else if(_exploreTab==='status'){
    filtered = publicPosts.filter(function(p){ return p.isStatus; });
  } else if(_exploreTab==='polls'){
    filtered = publicPosts.filter(function(p){ return p.isPoll; });
  } else {
    filtered = publicPosts.filter(function(p){ return !p.isStatus && !p.isPoll; }); // "Posts" = media posts, matching spec's category split
  }

  if(!filtered.length){ ef.innerHTML='<div class="empty">Nothing here yet.</div>'; return; }
  filtered.forEach((p,i)=>ef.appendChild(buildCard(p,i)));
}

// ── KEZ MEDIA OFFICIAL COMMUNITY (group page) ─────────
// Only one community exists for now — this constant is the seed for future multi-community support
// without changing the data shape (matches the Firestore layout described in the spec).
var KEZ_OFFICIAL_COMMUNITY_ID = 'kez-official';
var _communityMemberCache = {}; // communityId -> boolean (am I a member)

async function openCommunityPage(communityId){
  communityId = communityId || KEZ_OFFICIAL_COMMUNITY_ID;
  goTo('community');
  var container = document.getElementById('communityContent');
  container.innerHTML = '<div class="loading"><div class="spinner"></div>Loading community...</div>';

  var doc = await db.collection('communities').doc(communityId).get().catch(function(){return null;});
  var info = (doc && doc.exists) ? doc.data() : {
    name:'KEZ Media Official Community', description:'Welcome to the official KEZ Media community. Stay up to date with announcements, feature updates, sneak peeks, development progress, bug reports, feature suggestions, community events, and future projects. Create your own posts, join discussions, interact with other members, share feedback, and be part of the growing KEZ community.',
    memberCount:221, isOfficial:true, coverImage:'', avatarImage:''
  };
  // Seed the doc on first-ever load so future edits (cover/avatar/member count) have something to merge into
  if(!doc || !doc.exists){
    db.collection('communities').doc(communityId).set(_stripUndefined(info)).catch(function(){});
  }

  var memberDoc = await db.collection('communities').doc(communityId).collection('members').doc(me.uid).get().catch(function(){return null;});
  var isMember = !!(memberDoc && memberDoc.exists);
  _communityMemberCache[communityId] = isMember;

  var canEditBranding = isAdmin(); // only admin controls the official community's cover/pfp

  container.innerHTML =
    '<div class="profile-card">'
    +'<div style="position:relative;height:150px;background:'+(info.coverImage?'none':'linear-gradient(135deg,#e2688a,#a855f7)')+';">'
      +(info.coverImage?'<img src="'+esc(info.coverImage)+'" style="width:100%;height:100%;object-fit:cover;">':'')
      +'<button onclick="goTo(previousView||\'explore\')" style="position:absolute;top:12px;left:12px;width:32px;height:32px;border-radius:50%;background:rgba(0,0,0,.4);border:none;color:white;cursor:pointer;display:flex;align-items:center;justify-content:center;">'
        +'<svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg></button>'
      +(canEditBranding?'<button onclick="document.getElementById(\'communityCoverUpload\').click()" style="position:absolute;bottom:10px;right:10px;background:rgba(0,0,0,.5);border:none;color:white;font-size:11px;padding:6px 10px;border-radius:8px;cursor:pointer;">Change Cover</button>':'')
    +'</div>'
    +'<div style="padding:0 18px 18px;">'
      +'<div style="display:flex;align-items:flex-end;gap:14px;margin-top:-36px;">'
        +'<div style="position:relative;">'
        +'<div style="width:78px;height:78px;border-radius:50%;border:4px solid var(--card);overflow:hidden;background:'+(info.avatarImage?'none':'linear-gradient(135deg,#e2688a,#a855f7)')+';display:flex;align-items:center;justify-content:center;">'
          +(info.avatarImage?'<img src="'+esc(info.avatarImage)+'" style="width:100%;height:100%;object-fit:cover;">':'<svg width="30" height="30" fill="none" stroke="white" stroke-width="1.5" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>')
        +'</div>'
        +(canEditBranding?'<button onclick="document.getElementById(\'communityAvatarUpload\').click()" style="position:absolute;bottom:0;right:0;width:24px;height:24px;border-radius:50%;background:var(--pink);border:2px solid var(--card);color:white;font-size:11px;cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;"><svg width="12" height="12" fill="none" stroke="white" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg></button>':'')
        +'</div>'
      +'</div>'
      +'<div style="display:flex;align-items:center;gap:8px;margin-top:10px;">'
        +'<div style="font-size:18px;font-weight:800;">'+esc(info.name)+'</div>'
        +(info.isOfficial?'<div style="background:var(--pink);color:white;font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px;">OFFICIAL</div>':'')
      +'</div>'
      +'<div style="font-size:12.5px;color:var(--text3);margin:4px 0 12px;" id="communityMemberCount">'+(info.memberCount||0)+' members</div>'
      +'<button id="communityJoinBtn" style="width:100%;padding:11px;border-radius:12px;border:'+(isMember?'1.5px solid var(--border)':'none')+';background:'+(isMember?'none':'var(--pink)')+';color:'+(isMember?'var(--text)':'white')+';font-weight:600;font-size:13.5px;cursor:pointer;margin-bottom:14px;">'+(isMember?'Joined ✓':'Join Community')+'</button>'
      +'<div style="font-size:13px;color:var(--text2);line-height:1.6;">'+esc(info.description)+'</div>'
    +'</div>'
    +'</div>'
    +'<div id="communityCanEditBrandingFlag" style="display:none;" data-val="'+(canEditBranding?'1':'0')+'"></div>'
    +'<input type="file" id="communityCoverUpload" accept="image/*" style="display:none;" onchange="_uploadCommunityBranding(this,\''+communityId+'\',\'coverImage\')">'
    +'<input type="file" id="communityAvatarUpload" accept="image/*" style="display:none;" onchange="_uploadCommunityBranding(this,\''+communityId+'\',\'avatarImage\')">'
    +'<div class="search-type-tabs" id="communityTabs" style="flex-wrap:wrap;margin-top:10px;">'
      +'<button class="search-type-tab on" data-ctab="home" onclick="setCommunityTab(\''+communityId+'\',\'home\')">Home</button>'
      +'<button class="search-type-tab" data-ctab="announcements" onclick="setCommunityTab(\''+communityId+'\',\'announcements\')">Announcements</button>'
      +'<button class="search-type-tab" data-ctab="discussions" onclick="setCommunityTab(\''+communityId+'\',\'discussions\')">Discussions</button>'
      +'<button class="search-type-tab" data-ctab="media" onclick="setCommunityTab(\''+communityId+'\',\'media\')">Media</button>'
      +'<button class="search-type-tab" data-ctab="events" onclick="setCommunityTab(\''+communityId+'\',\'events\')">Events</button>'
      +'<button class="search-type-tab" data-ctab="members" onclick="setCommunityTab(\''+communityId+'\',\'members\')">Members</button>'
      +'<button class="search-type-tab" data-ctab="about" onclick="setCommunityTab(\''+communityId+'\',\'about\')">About</button>'
    +'</div>'
    +'<div id="communityComposerWrap" style="margin:14px 0;"></div>'
    +'<div id="communityFeed"></div>';

  document.getElementById('communityJoinBtn').onclick = function(){ toggleCommunityMembership(communityId); };
  _communityInfoCache[communityId] = info;
  _communityIsMemberFlag = isMember;
  renderCommunityComposer(communityId, isMember);
  loadCommunityFeed(communityId);
}

var _communityInfoCache = {};
var _communityIsMemberFlag = false;
var _communityCurrentTab = 'home';

function setCommunityTab(communityId, tab){
  _communityCurrentTab = tab;
  document.querySelectorAll('#communityTabs [data-ctab]').forEach(function(b){
    b.classList.toggle('on', b.dataset.ctab===tab);
  });
  var composerWrap = document.getElementById('communityComposerWrap');
  var isMember = _communityIsMemberFlag;

  if(tab==='home' || tab==='discussions'){
    composerWrap.style.display = '';
    renderCommunityComposer(communityId, isMember);
    loadCommunityFeed(communityId, tab==='discussions' ? {excludeAnnouncements:true} : {});
  } else if(tab==='announcements'){
    composerWrap.style.display = isAdmin() ? '' : 'none';
    if(isAdmin()) renderCommunityComposer(communityId, isMember, true);
    loadCommunityFeed(communityId, {onlyAnnouncements:true});
  } else if(tab==='media'){
    composerWrap.style.display = 'none';
    loadCommunityFeed(communityId, {onlyMedia:true});
  } else if(tab==='events'){
    composerWrap.style.display = 'none';
    renderCommunityEvents(communityId);
  } else if(tab==='members'){
    composerWrap.style.display = 'none';
    renderCommunityMembers(communityId);
  } else if(tab==='about'){
    composerWrap.style.display = 'none';
    renderCommunityAbout(communityId);
  }
}

function renderCommunityAbout(communityId){
  var info = _communityInfoCache[communityId] || {};
  var feedEl = document.getElementById('communityFeed');
  var createdLabel = 'Unknown';
  if(info.createdAt){
    var ms = info.createdAt.toMillis ? info.createdAt.toMillis() : (info.createdAt.seconds ? info.createdAt.seconds*1000 : null);
    if(ms) createdLabel = new Date(ms).toLocaleDateString(undefined,{month:'long',day:'numeric',year:'numeric'});
  }
  function row(label,value){
    return '<div style="display:flex;justify-content:space-between;padding:9px 0;border-bottom:1px solid var(--border);"><span style="font-size:12.5px;color:var(--text3);">'+label+'</span><span style="font-size:12.5px;font-weight:600;">'+value+'</span></div>';
  }
  var rules = ['Be respectful.','No spam.','No hate speech.','No illegal content.','Respect copyright.','Follow KEZ Media Community Guidelines.'];
  feedEl.innerHTML =
    '<div style="border:1px solid var(--border);border-radius:14px;padding:16px;margin-bottom:12px;">'
    +'<div style="font-size:13px;color:var(--text2);line-height:1.6;margin-bottom:14px;">'+esc(info.description||'')+'</div>'
    +row('Members', info.memberCount||0)
    +row('Created', createdLabel)
    +'</div>'
    +'<div style="border:1px solid var(--border);border-radius:14px;padding:16px;">'
    +'<div style="font-size:13px;font-weight:700;margin-bottom:8px;">Community Rules</div>'
    +'<ol style="margin:0;padding-left:18px;font-size:12.5px;color:var(--text2);line-height:2;">'+rules.map(function(r){return '<li>'+esc(r)+'</li>';}).join('')+'</ol>'
    +'</div>';
}

async function renderCommunityMembers(communityId){
  var feedEl = document.getElementById('communityFeed');
  feedEl.innerHTML = '<div class="loading"><div class="spinner"></div>Loading members...</div>';
  try{
    var snap = await db.collection('communities').doc(communityId).collection('members').orderBy('joinedAt','desc').get();
    if(!snap.docs.length){ feedEl.innerHTML='<div class="empty">No members yet — be the first to join!</div>'; return; }
    feedEl.innerHTML = '<input type="text" id="communityMemberSearch" placeholder="Search members..." oninput="_filterCommunityMembers(this.value)" style="width:100%;padding:10px 14px;border-radius:12px;border:1px solid var(--border);background:var(--bg2);color:var(--text);font-size:13px;box-sizing:border-box;margin-bottom:10px;">'
      + '<div id="communityMembersList"></div>';
    var listEl = document.getElementById('communityMembersList');
    var uids = snap.docs.map(function(d){return d.id;});
    function renderList(filter){
      var q = (filter||'').toLowerCase();
      listEl.innerHTML = uids.map(function(uid){
        var u = allUsers[uid] || {};
        var name = u.name || u.handle || 'Member';
        if(q && name.toLowerCase().indexOf(q)===-1 && (u.handle||'').toLowerCase().indexOf(q)===-1) return '';
        var av = u.avatar ? '<img src="'+esc(u.avatar)+'" style="width:100%;height:100%;object-fit:cover;">' : '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:'+(u.color||'var(--pink)')+';color:white;font-weight:700;">'+(u.initial||'?')+'</div>';
        return '<div onclick="viewProfile(\''+uid+'\')" style="display:flex;align-items:center;gap:10px;padding:10px 4px;cursor:pointer;border-bottom:1px solid var(--border);">'
          +'<div style="width:40px;height:40px;border-radius:50%;overflow:hidden;flex-shrink:0;">'+av+'</div>'
          +'<div><div style="font-size:13px;font-weight:600;display:flex;align-items:center;gap:3px;">'+esc(name)+verifiedBadgeHTML(u,12)+'</div><div style="font-size:11.5px;color:var(--text3);">@'+esc(u.handle||'')+'</div></div>'
          +'</div>';
      }).join('');
    }
    window._filterCommunityMembers = renderList;
    renderList('');
  }catch(e){ feedEl.innerHTML='<div class="empty">Could not load members.</div>'; }
}

async function renderCommunityEvents(communityId){
  var feedEl = document.getElementById('communityFeed');
  feedEl.innerHTML = '<div class="loading"><div class="spinner"></div>Loading events...</div>';
  try{
    var snap = await db.collection('communities').doc(communityId).collection('events').orderBy('date','asc').get();
    var events = snap.docs.map(function(d){return {id:d.id,...d.data()};});
    var adminBtn = isAdmin() ? '<button onclick="_promptCreateCommunityEvent(\''+communityId+'\')" style="width:100%;padding:10px;border-radius:10px;border:1.5px dashed var(--pink);background:none;color:var(--pink);font-weight:600;font-size:12.5px;cursor:pointer;margin-bottom:12px;">+ Create Event</button>' : '';
    if(!events.length){ feedEl.innerHTML = adminBtn+'<div class="empty">No events scheduled yet.</div>'; return; }
    feedEl.innerHTML = adminBtn + events.map(function(ev){
      return '<div style="border:1px solid var(--border);border-radius:14px;padding:14px;margin-bottom:10px;">'
        +'<div style="font-size:11px;color:var(--pink);font-weight:700;text-transform:uppercase;">'+esc(ev.date||'')+'</div>'
        +'<div style="font-size:14px;font-weight:700;margin:4px 0;">'+esc(ev.title||'Untitled Event')+'</div>'
        +'<div style="font-size:12.5px;color:var(--text2);">'+esc(ev.description||'')+'</div>'
        +'</div>';
    }).join('');
  }catch(e){ feedEl.innerHTML='<div class="empty">Could not load events.</div>'; }
}
function _promptCreateCommunityEvent(communityId){
  var title = prompt('Event title:');
  if(!title) return;
  var date = prompt('Event date (e.g. August 20, 2026):','');
  var description = prompt('Short description:','');
  db.collection('communities').doc(communityId).collection('events').add(_stripUndefined({
    title:title, date:date||'', description:description||'', createdBy:me.uid, createdAt:firebase.firestore.FieldValue.serverTimestamp()
  })).then(function(){ renderCommunityEvents(communityId); }).catch(function(){ toast('Could not create event'); });
}

function renderCommunityComposer(communityId, isMember, isAnnouncement){
  var wrap = document.getElementById('communityComposerWrap');
  if(!wrap) return;
  if(isAnnouncement && !isAdmin()){ wrap.innerHTML=''; return; } // only admin sees the announcement composer
  if(!isAnnouncement && !isMember){
    wrap.innerHTML = '<div style="text-align:center;padding:14px;color:var(--text3);font-size:12.5px;border:1px dashed var(--border);border-radius:12px;">Join the community to create posts.</div>';
    return;
  }
  wrap.innerHTML =
    '<div style="border:1px solid var(--border);border-radius:14px;padding:12px;">'
    +(isAnnouncement?'<div style="font-size:11px;font-weight:700;color:var(--pink);margin-bottom:6px;">POSTING AS ANNOUNCEMENT</div>':'')
    +'<textarea id="communityPostText" placeholder="'+(isAnnouncement?'Write an announcement...':'Share something with the community...')+'" style="width:100%;min-height:60px;border:none;outline:none;background:none;color:var(--text);font-family:inherit;font-size:13.5px;resize:none;box-sizing:border-box;"></textarea>'
    +'<div id="communityPostPreview" style="margin:6px 0;"></div>'
    +'<div style="display:flex;align-items:center;justify-content:space-between;margin-top:6px;">'
      +'<button onclick="document.getElementById(\'communityPostMedia\').click()" style="background:none;border:none;color:var(--text3);cursor:pointer;display:flex;align-items:center;gap:5px;font-size:12px;">'
        +'<svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg> Photo/Video</button>'
      +'<button onclick="submitCommunityPost(\''+communityId+'\','+(isAnnouncement?'true':'false')+')" style="background:var(--pink);color:white;border:none;border-radius:10px;padding:8px 18px;font-size:13px;font-weight:600;cursor:pointer;">'+(isAnnouncement?'Announce':'Post')+'</button>'
    +'</div>'
    +'<input type="file" id="communityPostMedia" accept="image/*,video/*" style="display:none;" onchange="_stageCommunityPostMedia(this)">'
    +'</div>';
}

var _stagedCommunityMedia = null;
function _stageCommunityPostMedia(inp){
  var f = inp.files[0];
  if(!f) return;
  _stagedCommunityMedia = {file:f, type:f.type.startsWith('video/')?'video':'image', previewUrl:URL.createObjectURL(f)};
  var preview = document.getElementById('communityPostPreview');
  preview.innerHTML = _stagedCommunityMedia.type==='video'
    ? '<video src="'+_stagedCommunityMedia.previewUrl+'" muted style="width:100%;max-height:180px;border-radius:10px;object-fit:cover;"></video>'
    : '<img src="'+_stagedCommunityMedia.previewUrl+'" style="width:100%;max-height:180px;border-radius:10px;object-fit:cover;">';
}

async function submitCommunityPost(communityId, isAnnouncement){
  var txt = document.getElementById('communityPostText').value.trim();
  if(!txt && !_stagedCommunityMedia){ toast('Write something or add a photo/video first'); return; }
  var postId = 'cpost_'+Date.now();
  var media = [];
  if(_stagedCommunityMedia){
    toast('Uploading...');
    var url = _stagedCommunityMedia.type==='video'
      ? await uploadFileToStorage(_stagedCommunityMedia.file, `communities/${communityId}/${Date.now()}.mp4`)
      : await uploadImageFile(_stagedCommunityMedia.file, `communities/${communityId}/${Date.now()}.jpg`);
    if(url) media.push({type:_stagedCommunityMedia.type, url:url});
  }
  var p = {
    id: postId, uid: me.uid, user:{...me}, media: media,
    images: media.filter(function(m){return m.type==='image';}).map(function(m){return m.url;}),
    image: null, caption: txt, likes:0, liked:false, comments:[], saved:false,
    time:'Just now', createdAt: Date.now(), showComments:false, _community: communityId,
    isAnnouncement: !!isAnnouncement
  };
  await savePostToFirestore(p);
  document.getElementById('communityPostText').value='';
  document.getElementById('communityPostPreview').innerHTML='';
  _stagedCommunityMedia = null;
  toast(isAnnouncement ? 'Announcement posted!' : 'Posted to the community!');
  loadCommunityFeed(communityId, isAnnouncement ? {onlyAnnouncements:true} : {});
}

async function loadCommunityFeed(communityId, filterOpts){
  filterOpts = filterOpts || {};
  var feedEl = document.getElementById('communityFeed');
  if(!feedEl) return;
  feedEl.innerHTML = '<div class="loading"><div class="spinner"></div>Loading posts...</div>';
  try{
    var snap = await communityPostsCol(communityId).orderBy('createdAt','desc').get();
    var cposts = snap.docs.map(function(d){ return {id:d.id, ...d.data(), _community:communityId}; });
    // Merge into the global posts array (tagged _community) so buildCard/doLike/addComment work unmodified.
    // Home feed and Explore both explicitly filter _community posts out, so this never leaks there.
    posts = posts.filter(function(p){ return p._community!==communityId; }); // avoid duplicating on reload
    posts = cposts.concat(posts);

    var display = cposts;
    if(filterOpts.onlyAnnouncements) display = cposts.filter(function(p){return p.isAnnouncement;});
    else if(filterOpts.excludeAnnouncements) display = cposts.filter(function(p){return !p.isAnnouncement;});
    else if(filterOpts.onlyMedia) display = cposts.filter(function(p){return p.media && p.media.length;});

    feedEl.innerHTML='';
    if(!display.length){
      var emptyMsg = filterOpts.onlyAnnouncements ? 'No announcements yet.' : filterOpts.onlyMedia ? 'No photos or videos shared yet.' : 'No posts yet — be the first to share something!';
      feedEl.innerHTML='<div class="empty">'+emptyMsg+'</div>';
      return;
    }
    display.forEach(function(p,i){ feedEl.appendChild(buildCard(p,i)); });
  }catch(e){ feedEl.innerHTML='<div class="empty">Could not load community posts.</div>'; }
}

async function toggleCommunityMembership(communityId){
  var isMember = _communityMemberCache[communityId];
  var memberRef = db.collection('communities').doc(communityId).collection('members').doc(me.uid);
  var communityRef = db.collection('communities').doc(communityId);
  try{
    if(isMember){
      await memberRef.delete();
      await communityRef.update({memberCount: firebase.firestore.FieldValue.increment(-1)});
      _communityMemberCache[communityId] = false;
      toast('Left the community');
    } else {
      await memberRef.set({uid:me.uid, joinedAt: firebase.firestore.FieldValue.serverTimestamp()});
      await communityRef.update({memberCount: firebase.firestore.FieldValue.increment(1)});
      _communityMemberCache[communityId] = true;
      toast('Joined the community!');
    }
    openCommunityPage(communityId); // simplest reliable way to refresh header + composer state together
  }catch(e){ toast('Could not update membership'); }
}

async function _uploadCommunityBranding(inp, communityId, field){
  var f = inp.files[0];
  if(!f) return;
  toast('Uploading...');
  var url = await uploadImageFile(f, `communities/${communityId}/branding_${field}_${Date.now()}.jpg`);
  if(!url){ toast('Upload failed'); return; }
  await db.collection('communities').doc(communityId).set(_stripUndefined({[field]: url}), {merge:true});
  toast('Updated!');
  openCommunityPage(communityId);
}

// Minimal KEZ Media Official Community card shown in Explore → Communities.
function buildCommunityCard(){
  var card = document.createElement('div');
  card.style.cssText='border:1px solid var(--border);border-radius:16px;overflow:hidden;margin:8px 4px;background:var(--card);';
  card.innerHTML =
    '<div style="height:90px;background:linear-gradient(135deg,#e2688a,#a855f7);"></div>'
    +'<div style="padding:16px;">'
    +'<div style="display:flex;align-items:center;gap:8px;">'
      +'<div style="font-size:16px;font-weight:800;">KEZ Media Official Community</div>'
      +'<div style="background:var(--pink);color:white;font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px;">OFFICIAL</div>'
    +'</div>'
    +'<div style="font-size:12px;color:var(--text3);margin:6px 0 10px;">221 members</div>'
    +'<div style="font-size:13px;color:var(--text2);line-height:1.5;margin-bottom:14px;">Welcome to the official KEZ Media community. Stay up to date with announcements, feature updates, sneak peeks, development progress, bug reports, feature suggestions, community events, and future projects.</div>'
    +'<button onclick="openCommunityPage(\'kez-official\')" style="width:100%;padding:11px;border-radius:12px;border:none;background:var(--pink);color:white;font-weight:600;font-size:13.5px;cursor:pointer;">View Community</button>'
    +'</div>';
  return card;
}

// Builds feed-card HTML for a post's mixed media array (photos + videos, any order, any count)
function _buildMixedMediaHTML(pid, mediaArr, cardIndex){
  if(mediaArr.length===1){
    const m=mediaArr[0];
    if(m.type==='video'){
      return '<div class="carousel-wrap"><video class="video-preview" src="'+esc(m.url)+'" controls playsinline style="width:100%;max-height:520px;display:block;background:#000;border-radius:12px;"></video></div>';
    }
    return '<div class="carousel-wrap"><img class="card-img-click" data-src="'+encodeURIComponent(m.url)+'" style="width:100%;max-height:520px;object-fit:cover;display:block;cursor:pointer;" src="'+m.url+'" alt="post"></div>';
  }
  let slides='',dots='';
  mediaArr.forEach(function(m){
    if(m.type==='video'){
      slides+='<div class="carousel-slide"><video class="video-preview" src="'+esc(m.url)+'" controls playsinline style="width:100%;max-height:520px;display:block;background:#000;"></video></div>';
    } else {
      slides+='<div class="carousel-slide"><img class="card-img-click" data-src="'+encodeURIComponent(m.url)+'" src="'+m.url+'" alt="post"></div>';
    }
  });
  mediaArr.forEach(function(_,di){dots+='<span class="carousel-dot '+(di===0?'active':'')+'" data-slide="'+di+'"></span>';});
  const vidCount=mediaArr.filter(function(m){return m.type==='video';}).length;
  const imgCount=mediaArr.length-vidCount;
  const badge=(vidCount&&imgCount)?('📷'+imgCount+' 🎥'+vidCount):mediaArr.length;
  // No auto-slide for mixed carousels — avoids interrupting video playback
  return '<div class="carousel-wrap" id="car-'+pid+'" data-pid="'+pid+'" data-count="'+mediaArr.length+'" data-idx="0">'
    ...
    +'<div class="multi-badge">'+badge+'</div>'
    +'</div>';
}
...
function buildCard(p,i){
  ...
  if(p.isStatus){
    ...
    mediaHTML = _buildPollHTML(pid, p);
  } else if(p.media && p.media.length){
    mediaHTML = _buildMixedMediaHTML(pid, p.media, i);
    var _hasVid = p.media.some(function(m){return m.type==='video';});
    if(!_hasVid && p.media.length>1){
      setTimeout(function(){startFeedAutoSlide(pid,p.media.length);},600+i*80);
    }
  ...
  let linkPreviewHTML = '';
  if(p.linkPreview){
    linkPreviewHTML = '<div class="post-link-preview">'+buildLinkPreviewHTML(p.linkPreview)+'</div>';
  } else if(p.caption && extractFirstUrl(p.caption)){
    // Not cached yet — empty placeholder that gets filled in after the card mounts (see wiring below)
    linkPreviewHTML = '<div class="post-link-preview" data-pid="'+pid+'" data-needs-preview="1"></div>';
  }
  const captionRow=(!p.isStatus&&p.caption)
    ?'<div class="post-content"><div class="post-caption">'+linkifyCaption(p.caption)+(p.edited?'<span style="font-size:10.5px;color:var(--text3);margin-left:6px;font-weight:400;">(edited)</span>':'')+'</div>'+linkPreviewHTML+'</div>'
    :'';
  ...
  card.innerHTML=
    '<div class="post-header">'
      ...
      +'<div class="post-meta">'
        +'<div class="post-username card-open-profile" data-uid="'+(p.uid||'')+'" style="cursor:pointer;display:inline-flex;align-items:center;gap:4px;">@'+userHandle+verifiedBadgeHTML(liveUser,14)+'</div>'
  ...
  // Fetch a rich link preview for posts whose caption has a URL but no cached preview yet.
  // Once fetched, it's persisted to Firestore so this never re-fetches for anyone else viewing it.
  var _lpEl = card.querySelector('[data-needs-preview="1"]');
  if(_lpEl){
    renderLinkPreviewInto(_lpEl, p.caption, postsCol().doc(String(pid)), 'linkPreview');
  }
  card.querySelectorAll('.c-link-preview[data-comment-url]').forEach(function(el){
    renderLinkPreviewInto(el, el.dataset.commentUrl, null, null); // not persisted — comments are inside a nested array, re-fetches each view but stays in-memory cached for the session
  });
  // Double-tap / double-click to like (Instagram-style) — works over the whole media area,
  // including photos, videos, and mixed-media carousels. Always shows the big heart burst;
  // only actually likes the post if it wasn't already liked (never unlikes on double-tap).
  var mediaAreaForDblTap = card.querySelector('.carousel-wrap') || card.querySelector('.status-body');
  if(mediaAreaForDblTap){
    mediaAreaForDblTap.style.position = mediaAreaForDblTap.style.position || 'relative';
    mediaAreaForDblTap.addEventListener('dblclick', function(e){
      e.preventDefault();
      _bigHeartBurst(mediaAreaForDblTap);
      var pp = posts.find(function(x){return String(x.id)===String(pid);});
      if(pp && !pp.liked) doLike(pid);
    });
    var _lastTapT = 0;
    mediaAreaForDblTap.addEventListener('touchend', function(e){
      var now = Date.now();
      if(now - _lastTapT < 350){
        e.preventDefault();
        _bigHeartBurst(mediaAreaForDblTap);
        var pp2 = posts.find(function(x){return String(x.id)===String(pid);});
        if(pp2 && !pp2.liked) doLike(pid);
      }
      _lastTapT = now;
    });
  }
  ...
  var hasVideoSlide = !!(p.media && p.media.some(function(m){return m.type==='video';}));
  if(wrap && !hasVideoSlide){
...
function commentHTML(c, postId, commentIdx){
  ...
  return '<div class="comment" id="comment-'+String(postId)+'-'+commentIdx+'">'
    ...
    +'<div style="flex:1;">'
      +'<div class="c-bubble">'
        +'<div class="c-user" style="display:inline-flex;align-items:center;gap:3px;'+(uid?'cursor:pointer;color:var(--pink);':'')+'"'+(uid?' onclick="viewProfile(_g('+iU+'))"':'')+'>@'+esc(handle)+verifiedBadgeHTML(live||u,12)+'</div>'
        ...
        +(!c.gifUrl?'<div class="c-text">'+linkifyCaption(c.text||'')+'</div>'+(c.text&&extractFirstUrl(c.text)?'<div class="c-link-preview" data-comment-url="'+esc(extractFirstUrl(c.text))+'" id="clp-'+String(postId)+'-'+commentIdx+'"></div>':''):'')
      ...
      +'</div>'
      // Action row: Like count + React emoji + Reply
      +'<div style="display:flex;align-items:center;gap:12px;margin-top:4px;margin-left:2px;">'
        +'<button class="c-like-btn'+(myLike?' active':'')+'" data-pid="'+String(postId)+'" data-idx="'+commentIdx+'" onclick="likeComment(\''+String(postId)+'\','+commentIdx+')" style="background:none;border:none;cursor:pointer;font-size:11.5px;color:'+(myLike?'var(--pink)':'var(--text3)')+';display:flex;align-items:center;gap:3px;font-family:Jost,sans-serif;padding:0;position:relative;">'
      ...
      +(postId?'<div id="reply-input-'+String(postId)+'-'+commentIdx+'" style="display:none;margin-top:6px;"><div style="display:flex;gap:6px;align-items:center;"><input class="c-input" id="reply-inp-'+String(postId)+'-'+commentIdx+'" placeholder="Reply to @'+esc(handle)+'..." style="flex:1;font-size:12.5px;padding:6px 10px;"><button onclick="submitReply(\''+String(postId)+'\','+commentIdx+')" style="background:var(--pink);border:none;border-radius:8px;padding:6px 10px;color:white;font-size:12px;cursor:pointer;font-family:Jost,sans-serif;">Post</button></div></div>':'')
    +'</div>'
  +'</div>';
}

// ── INTERACTIONS ──────────────────────────────────────
// Big center-screen heart burst for double-tap-to-like (Instagram-style)
function _bigHeartBurst(container){
  var heart=document.createElement('div');
  heart.className='big-heart-burst';
  heart.innerHTML='<svg width="90" height="90" viewBox="0 0 24 24" fill="white" style="filter:drop-shadow(0 2px 10px rgba(0,0,0,.35));"><path d="M12 21s-7.5-4.6-10-9.3C.4 8.4 2 5 5.4 5c2 0 3.4 1.1 4.3 2.4.1.1.2.1.3 0C11 6.1 12.4 5 14.4 5 17.8 5 19.6 8.4 22 11.7 14.5 16.4 12 21 12 21z"/></svg>';
  heart.style.cssText='position:absolute;top:50%;left:50%;transform:translate(-50%,-50%) scale(.4);opacity:0;pointer-events:none;z-index:20;';
  container.appendChild(heart);
  requestAnimationFrame(function(){
    heart.style.transition='transform .35s cubic-bezier(.17,.89,.32,1.49),opacity .25s ease';
    heart.style.transform='translate(-50%,-50%) scale(1.1)';
    heart.style.opacity='1';
    setTimeout(function(){
      heart.style.transition='opacity .3s ease, transform .3s ease';
      heart.style.opacity='0';
      heart.style.transform='translate(-50%,-50%) scale(1.3)';
      setTimeout(function(){ if(heart.parentNode) heart.parentNode.removeChild(heart); },320);
    },450);
  });
}
function doLike(id){
  ...
  if(alreadyLiked){
    ...
    if(p.uid && p.uid!==me.uid) sendNotification(p.uid,'like',me,'',id);
    me.likesGivenCount = (me.likesGivenCount||0)+1;
    db.collection('profiles').doc(me.uid).set({likesGivenCount: firebase.firestore.FieldValue.increment(1)},{merge:true}).catch(function(){});
    checkAchievements();
  }
  // update UI
...
function addComment(id){
  ...
  async function doAddComment(imageUrl){
    if(replyToIdx>=0 && p.comments[replyToIdx]){
      ...
      me.repliesMadeCount = (me.repliesMadeCount||0)+1;
      db.collection('profiles').doc(me.uid).set({repliesMadeCount: firebase.firestore.FieldValue.increment(1)},{merge:true}).catch(function(){});
      checkAchievements();
    } else {
      ...
      sendMentionNotifications(txt,id);
      me.commentsMadeCount = (me.commentsMadeCount||0)+1;
      db.collection('profiles').doc(me.uid).set({commentsMadeCount: firebase.firestore.FieldValue.increment(1)},{merge:true}).catch(function(){});
      checkAchievements();
    }
    // Clear photo preview
...
function sendMentionNotifications(text, postId){
  ...
  Object.values(allUsers).forEach(function(u){
    ...
    if(mentions.includes(u.handle.toLowerCase())){
      notified.add(u.uid);
      sendNotification(u.uid, 'mention', me, '@'+u.handle+' — '+text.slice(0,50), postId);
...
function deletePost(id){
  ...
  // 3b. Save full copy to deletedPosts (Trash) for admin recovery + Cloudinary tracking
  var _dpPost=posts.find(function(x){return String(x.id)===id;});
  if(_dpPost){
    var _dpMedia=[];
    var _dpImgs=_dpPost.images||(_dpPost.image?[_dpPost.image]:[]);
    _dpImgs.forEach(function(u){if(u)_dpMedia.push({url:u,type:'post-image',publicId:u.split('/upload/')[1]||''});});
    if(_dpPost.videoUrl)_dpMedia.push({url:_dpPost.videoUrl,type:'post-video',publicId:_dpPost.videoUrl.split('/upload/')[1]||''});
    ['pollPhotoA','pollPhotoB','pollPhotoC'].forEach(function(k){if(_dpPost[k])_dpMedia.push({url:_dpPost[k],type:'poll-photo',publicId:_dpPost[k].split('/upload/')[1]||''});});
    var _dpU=_dpPost.user||allUsers[_dpPost.uid]||{};
    db.collection('deletedPosts').add({
      originalPostId:id,
      uid:_dpPost.uid||'',
      userName:_dpU.name||me.name||'',
      userHandle:_dpU.handle||me.handle||'',
      caption:_dpPost.caption||'',
      isPoll:!!_dpPost.isPoll,
      isStatus:!!_dpPost.isStatus,
      isVideo:!!_dpPost.isVideo,
      postType:_dpPost.isPoll?'poll':_dpPost.isStatus?'status':_dpPost.isVideo?'video':'photo',
      pollQuestion:_dpPost.pollQuestion||null,
      mediaUrls:_dpMedia,
      mediaUrl:_dpMedia.length?_dpMedia[0].url:'',
      likes:_dpPost.likes||0,
      comments:(_dpPost.comments||[]).length,
      cloudinaryFolder:'kez-media/posts/'+(_dpPost.uid||'unknown'),
      deletedAt:firebase.firestore.FieldValue.serverTimestamp(),
      deletedBy:me.uid||'',
      deletedByHandle:me.handle||'',
      deletedBySelf:(_dpPost.uid===me.uid)
    }).catch(function(){});
  }
...
// ── POLL VOTE ─────────────────────────────────────────
// Real votePoll implementation is below (search "async function votePoll")
// No stub here — stub was removed because it blocked the real implementation

// ── PROFILE PAGE ──────────────────────────────────────
function refreshProfile(){
  applyUserProfile();
  const myPosts=posts.filter(p=>p.uid===me.uid && !p._community);
  document.getElementById('statPosts').textContent=myPosts.length;
  renderPostGrid();renderSavedGrid();renderStatusList();
  loadRecentVisitors(me.uid,'myRecentVisitors');
  renderPinnedBadges();
  loadManualBadges(me.uid,'myManualBadges',true);
}
// Manual admin-granted badges (separate system from automatic achievements).
// Firestore: profiles/{uid}/badges/{badgeId} → {badgeId, reason, grantedAt, expiresAt, visibility}
// Definitions live in badgeDefinitions/{badgeId} — fetched fresh here since the main app
// doesn't otherwise load that admin-only collection.
// ── MANUAL BADGE ICON LIBRARY (colorful SVG, no emoji) ──
// Mirrors the shape set used in the admin dashboard's badge creator, so a badge looks
// identical wherever it's rendered — admin panel or here on a user's profile.
var BADGE_ICON_PATHS = {
  trophy: '<path d="M8 21h8"/><path d="M12 17v4"/><path d="M7 4h10v5a5 5 0 0 1-10 0z"/><path d="M17 5h3a2 2 0 0 1-2 4"/><path d="M7 5H4a2 2 0 0 0 2 4"/>',
  medal:  '<circle cx="12" cy="15" r="6"/><path d="M9 10.5L6 3h3l3 6.5L15 3h3l-3 7.5"/>',
  crown:  '<path d="M2 18h20l-2-9-5 4-3-7-3 7-5-4z"/>',
  gem:    '<path d="M6 3h12l4 6-10 12L2 9z"/><path d="M2 9h20"/><path d="M9 3l3 6-3 12"/><path d="M15 3l-3 6 3 12"/>',
  ...
  sparkle:'<path d="M12 2l1.5 5.5L19 9l-5.5 1.5L12 16l-1.5-5.5L5 9l5.5-1.5z"/><path d="M19 15l.7 2.3L22 18l-2.3.7L19 21l-.7-2.3L16 18l2.3-.7z"/>',
  flag:   '<line x1="4" y1="22" x2="4" y2="2"/><path d="M4 4h13l-2.5 4L17 12H4"/>',
  bolt:   '<polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>',
  check:  '<circle cx="12" cy="12" r="10"/><polyline points="8 12 11 15 16 9"/>'
};
function badgeIconHTML(shapeKey, hexColor, size){
  var s = size||34;
  var color = hexColor || '#e2688a';
  var pathData = BADGE_ICON_PATHS[shapeKey] || BADGE_ICON_PATHS.trophy;
  return '<div style="width:'+s+'px;height:'+s+'px;border-radius:50%;background:linear-gradient(135deg,'+color+',#7c3aed);display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 2px 8px '+color+'55;">'
    + '<svg width="'+Math.round(s*0.55)+'" height="'+Math.round(s*0.55)+'" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">'+pathData+'</svg>'
    + '</div>';
}

async function loadManualBadges(uid, containerId, isOwner){
  var wrap = document.getElementById(containerId);
  if(!wrap) return;
  wrap.innerHTML='';
  try{
    var badgesSnap = await db.collection('profiles').doc(uid).collection('badges').get();
    var myBadges = badgesSnap.docs.map(function(d){return {id:d.id,...d.data()};})
      .filter(function(b){ return isOwner || b.visibility==='public'; });
    if(!myBadges.length) return;
    var defsSnap = await db.collection('badgeDefinitions').get();
    var defsById = {};
    defsSnap.docs.forEach(function(d){ defsById[d.id] = d.data(); });
    wrap.innerHTML = myBadges.map(function(b){
      var def = defsById[b.badgeId] || {};
      var title = (def.name||'Badge') + (b.reason?(' — '+b.reason):'');
      return '<div title="'+esc(title)+'">'+badgeIconHTML(def.icon, def.color, 34)+'</div>';
    }).join('');
...
function renderPostGrid(){
  ...
  var myPosts=posts.filter(p=>p.uid===me.uid && !p._community && !p.isStatus && !p.isPoll);
  ...
  myPosts.forEach(p=>{
    ...
    // Mixed media post (photo + video, any order) — uploaded via the Photo/Video tab
    else if(p.media && p.media.length){
      var _gfirst=p.media[0];
      var _mVid=p.media.filter(function(m){return m.type==='video';}).length;
      var _mImg=p.media.length-_mVid;
      var _mixedMultiIcon = `<div class="grid-multi-icon"><svg width="12" height="12" fill="none" stroke="white" stroke-width="2" viewBox="0 0 24 24"><rect x="2" y="7" width="15" height="15" rx="2"/><path d="M17 2H22V17"/></svg></div>`;
      var _mixedIcon = (_mVid&&_mImg) ? `<div class="grid-multi-icon" style="width:auto;padding:2px 6px;font-size:10px;color:#fff;white-space:nowrap;" title="${_mImg} photo, ${_mVid} video">📷${_mImg} 🎥${_mVid}</div>` : (p.media.length>1?_mixedMultiIcon:'');
      if(_gfirst.type==='video'){
        item.innerHTML = '<div style="position:relative;width:100%;height:100%;background:#111;overflow:hidden;">'
          + '<video src="'+esc(_gfirst.url)+'" preload="metadata" muted playsinline style="width:100%;height:100%;object-fit:cover;display:block;pointer-events:none;"></video>'
          + _mixedIcon
          ...
          + '</div>'
          + '</div>' + editBtn;
        var _gVidEl = item.querySelector('video');
        if(_gVidEl){
          _gVidEl.addEventListener('loadedmetadata', function(){ _gVidEl.currentTime = Math.min(1, _gVidEl.duration * 0.1 || 1); });
        }
      } else {
        item.innerHTML=`<img src="${_gfirst.url}" alt="">${_mixedIcon}<div class="grid-overlay"><svg width="15" height="15" fill="white" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg><span>${p.likes}</span><svg width="15" height="15" fill="none" stroke="white" stroke-width="2.2" viewBox="0 0 24 24"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg><span>${p.comments.length}</span></div>${editBtn}`;
      }
    } else if(p.isVideo && p.videoUrl){
...
function switchTab(el,tabId){
  ...
  if(tabId==='poll-tab') renderPollsTab(me.uid);
}

function renderPollsTab(forUid){
  var tab=document.getElementById('poll-tab');if(!tab)return;
  var targetUid = forUid || me.uid;
  var myPolls=posts.filter(function(p){ return p.uid===targetUid && p.isPoll && !p.hidden; });
    ...
    .then(function(snap){
      ...
      snap.docs.forEach(function(d){
        ...
        item.innerHTML = '<div class="sug-avatar" style="background:'+avBg+';color:white;overflow:hidden;">'+avHTML+'</div>'
          + '<div class="sug-info"><div class="sug-name" style="display:flex;align-items:center;gap:3px;">'+esc(u.name||'User')+verifiedBadgeHTML(_liveUserFor(u.uid,u),12)+'</div><div class="sug-sub">@'+esc(u.handle||'')+'</div></div>'
...
function openModal(){
  ...
  uploadImages=[];uploadMediaItems=[];selectedMood='';postMode='status';uploadVideoFile=null;currentYtData=null;
...
function handleFiles(inp){
  ...
  files.forEach(f=>{
    ...
    // Both images and videos: keep the raw File for upload, use a lightweight object URL for the
    // preview only. Images used to be read fully into a base64 string here — with 20+ full-res
    // phone photos that's easily 100+ MB held in memory at once, enough to freeze or silently
    // crash the tab on mobile before the post ever reaches Firestore. This avoids that entirely.
    const item={type:isVideo?'video':'image', file:f, previewUrl:URL.createObjectURL(f)};
    uploadMediaItems.push(item);
...
function renderUploadPreviews(){
  ...
  uploadMediaItems.forEach((item,i)=>{
    ...
    if(item.type==='video'){
      w.innerHTML='<video class="upload-thumb" src="'+item.previewUrl+'" muted playsinline></video>'
        +'<div class="upload-thumb-badge" style="position:absolute;bottom:4px;left:4px;background:rgba(0,0,0,.6);color:#fff;font-size:10px;padding:2px 6px;border-radius:6px;pointer-events:none;">🎥 Video</div>'
        +'<button class="remove-thumb" onclick="removeUploadImg('+i+')">✕</button>';
      w.style.position='relative';
    } else {
      w.innerHTML='<img class="upload-thumb" src="'+item.previewUrl+'"><button class="remove-thumb" onclick="removeUploadImg('+i+')">✕</button>';
    }
    wrap.appendChild(w);
  });
  // No cap — add as many photos/videos as you want, in any mix, any order
...
function removeUploadImg(i){
  const item=uploadMediaItems[i];
  if(item && item.previewUrl) URL.revokeObjectURL(item.previewUrl);
  uploadMediaItems.splice(i,1);
  if(!uploadMediaItems.length)document.getElementById('uploadZone').style.display='block';
  renderUploadPreviews();
}
...
async function submitPost(){
  ...
  if(postMode==='status'){
    ...
    if(!uploadMediaItems.length){toast('Add a photo or video first');return;}
    closeModal();
    const vidCount=uploadMediaItems.filter(m=>m.type==='video').length;
    const imgCount=uploadMediaItems.length-vidCount;
    toast(vidCount && imgCount ? 'Uploading photos & videos... 🌸' : vidCount ? 'Uploading video... 🎬' : 'Uploading photo... 🌸');
    // Upload every item in the exact order the user added them (photo, video, photo, photo...)
    const media=[];
    for(let i=0;i<uploadMediaItems.length;i++){
      const item=uploadMediaItems[i];
      try{
        if(item.type==='video'){
          const url=await uploadFileToStorage(item.file, `posts/${uid}/${Date.now()}_${i}.mp4`);
          if(url) media.push({type:'video', url});
        } else {
          const url=await uploadImageFile(item.file, `posts/${uid}/${Date.now()}_${i}.jpg`);
          if(url) media.push({type:'image', url});
        }
      }catch(e){}
    }
    if(!media.length){toast('Upload failed, please try again.');uploadMediaItems=[];return;}
    const imageUrls=media.filter(m=>m.type==='image').map(m=>m.url);
    const firstVideo=media.find(m=>m.type==='video');
    const p={id:postId, uid, user:{...me}, media, images:imageUrls, image:imageUrls[0]||null,
      videoUrl:(imageUrls.length===0 && firstVideo)?firstVideo.url:null, // back-compat only for pure-video single posts
      caption:cap||'', likes:0, liked:false, comments:[], saved:false, time:'Just now', createdAt:Date.now(), showComments:false};
    await savePostToFirestore(p);
    if(cap) sendMentionNotifications(cap, postId);
    db.collection('activityLog').add({type:'post',uid:me.uid,handle:me.handle,postId,caption:(cap||'').slice(0,100),mediaCount:media.length,images:imgCount,videos:vidCount,ts:firebase.firestore.FieldValue.serverTimestamp()}).catch(()=>{});
    toast(vidCount && imgCount ? 'Post shared! 🌸🎬' : vidCount ? 'Video shared! 🎬' : 'Post shared! 🌸');
    checkAchievements();
    uploadMediaItems=[];
    ...
    // pollOptions MUST be [{text,votes}] objects — plain strings break votePoll counting
    var pollOptionsObj=opts.map(function(o){return {text:o,votes:0};});
    // pollVotes MUST be {} (uid→index map) — old string-keyed format breaks counting
    const p={id:postId, uid, user:{...me}, images:[], image:null, isPoll:true,
      caption:pcap||q, pollQuestion:q,
      pollOptions:pollOptionsObj,
      pollVotes:{},
      pollEnds:endsAt,
      pollEndsAt:endsAt,
...
function openLightbox(pid){
  ...
  // ── Mixed media post (photos + videos, any order) ──
  if(p.media && p.media.length){
    if(p.media.length===1){
      var _m=p.media[0];
      imgSide = _m.type==='video'
        ? `<div class="lightbox-img-side" style="background:#000;display:flex;align-items:center;justify-content:center;padding:0;"><video src="${esc(_m.url)}" controls playsinline style="width:100%;max-height:90vh;display:block;"></video></div>`
        : `<div class="lightbox-img-side"><img src="${_m.url}" alt="" style="width:100%;height:100%;object-fit:contain;"></div>`;
    } else {
      const mSlides=p.media.map(function(m){
        return m.type==='video'
          ? `<div class="carousel-slide" style="min-width:100%;"><video src="${esc(m.url)}" controls playsinline style="width:100%;max-height:90vh;object-fit:contain;display:block;background:#000;"></video></div>`
          : `<div class="carousel-slide" style="min-width:100%;"><img src="${m.url}" style="width:100%;max-height:90vh;object-fit:contain;display:block;"></div>`;
      }).join('');
      const mDots=p.media.map((_,di)=>`<span class="carousel-dot ${di===0?'active':''}" onclick="goSlide('lbcar',${di})"></span>`).join('');
      imgSide=`<div class="lightbox-img-side" style="overflow:hidden;"><div id="lbcar" data-idx="0" style="position:relative;height:100%;overflow:hidden;"><div class="carousel-track" id="ctlbcar" style="display:flex;height:100%;">${mSlides}</div><button class="carousel-btn carousel-prev" onclick="shiftSlide('lbcar',-1)">&#8249;</button><button class="carousel-btn carousel-next" onclick="shiftSlide('lbcar',1)">&#8250;</button><div class="carousel-dots" style="position:absolute;bottom:10px;left:50%;transform:translateX(-50%);display:flex;gap:5px;">${mDots}</div></div></div>`;
    }
  }
  // ── YouTube post ──
  else if(p.isYoutube && p.ytVideoId){
...
function _drawOtherProfile(uid, prof){
  try{
    ...
    var isAway      = getPresenceStatus(uid) === 'away';
    var userPosts   = posts.filter(function(p){ return p.uid===uid && !p._community; });
    var photoPosts  = userPosts.filter(function(p){ return !p.isStatus && !p.isPoll; });
    var statusPosts = userPosts.filter(function(p){ return p.isStatus; });
    var pollPosts   = userPosts.filter(function(p){ return p.isPoll && !p.hidden; });
    var iUid        = _r(uid);
        ...
        +'<div class="opu-top">'
          +'<div class="profile-av-wrap" style="margin-top:-44px;margin-bottom:10px;">'
            ...
            +'<div class="profile-actions">'
              ...
              +'<div id="opuRecentVisitors" style="display:flex;justify-content:flex-end;margin-top:8px;"></div>'
            +'</div>'
          +'</div>'
          ...
          +(isOnline?'<div class="opu-online-badge" style="display:inline-flex;align-items:center;gap:4px;font-size:11px;color:#22c55e;font-weight:600;margin-bottom:4px;">● Active now</div>':isAway?'<div class="opu-online-badge" style="display:inline-flex;align-items:center;gap:4px;font-size:11px;color:#f59e0b;font-weight:600;margin-bottom:4px;">🌙 Away</div>':'')
          +'<div style="display:flex;align-items:center;justify-content:flex-start;gap:6px;">'
            +'<div class="profile-name" style="display:flex;align-items:center;gap:5px;">'+esc(profName)+(prof.isPrivate?'<svg width="13" height="13" fill="none" stroke="var(--text3)" stroke-width="2" viewBox="0 0 24 24" title="Private account"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>':'')+'</div>'
            +(isAdminProfile||prof.verified?'<div class="verified-inline" title="Verified">'+verifiedBadgeHTML({verified:true},22)+'</div>':'')
          +'</div>'
          +'<div class="profile-handle">@'+esc(prof.handle||'user')+'</div>'
          +(prof.bio?'<div class="profile-bio">'+esc(prof.bio)+'</div>':'')
          +'<button onclick="openProfileInfoPanel(\''+esc(uid)+'\')" style="background:none;border:1px solid var(--border);color:var(--text3);border-radius:20px;padding:3px 10px;font-size:11px;cursor:pointer;margin-bottom:8px;display:inline-flex;align-items:center;gap:4px;">'
            +'<svg width="11" height="11" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg> Profile Info</button>'
          +(Array.isArray(prof.pinnedAchievements)&&prof.pinnedAchievements.length
            ? '<div style="display:flex;gap:6px;margin-bottom:8px;">'+prof.pinnedAchievements.map(function(id){
                var def=ACHIEVEMENT_DEFS.find(function(d){return d.id===id;});
                if(!def) return '';
                return '<div title="'+esc(def.name)+'">'+achIconHTML(def.icon, def.cat, 34, true)+'</div>';
              }).join('')+'</div>'
            : '')
          +'<div id="opuManualBadges" style="display:flex;gap:6px;margin-bottom:8px;"></div>'
          ...
          +'</div>'
          // Story Highlights — public, read-only viewer of this user's highlights
          +'<div id="opuHighlightsSection" style="margin-top:12px;"><div id="opuHighlights"></div></div>'
          +'<div id="opuMutualFriends" style="padding:0 16px;margin-top:4px;"></div>'
        +'</div>'
      +'</div>'
      ...
      +'<div class="profile-card" style="overflow:hidden;">'
        +'<div class="profile-tabs-bar">'
          +'<div class="profile-tabs" id="opuTabs">'
            ...
            +'<div class="ptab" data-tab="polls">Polls</div>'
          +'</div>'
    ...
    // Story Highlights — public, read-only (Story Archive stays private to the owner)
    // Skipped entirely for locked private accounts, matching Instagram behavior.
    if(!isPrivateLocked){
      loadOtherProfileHighlights(uid);
      loadMutualFriends(uid);
    }
    recordProfileVisit(uid);
    loadRecentVisitors(uid);
    loadManualBadges(uid,'opuManualBadges',false);
    ...
    var isPrivateLocked = !!prof.isPrivate && !isFollowing && uid!==me.uid && !isAdmin();
    function renderOpuTab(tab){
      currentOpuTab = tab;
      var fc = document.getElementById('opuFeedContent');
      if(!fc) return;
      if(isPrivateLocked){
        fc.innerHTML = '<div style="display:flex;flex-direction:column;align-items:center;padding:50px 20px;text-align:center;">'
          +'<div style="width:56px;height:56px;border-radius:50%;background:var(--bg2);display:flex;align-items:center;justify-content:center;margin-bottom:14px;">'
          +'<svg width="24" height="24" fill="none" stroke="var(--text3)" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>'
          +'</div>'
          +'<div style="font-size:14.5px;font-weight:700;margin-bottom:4px;">This account is private</div>'
          +'<div style="font-size:12.5px;color:var(--text3);">Follow '+esc(profName)+' to see their photos, videos, and highlights.</div>'
          +'</div>';
        return;
      }
      ...
      var emptyLabel = tab==='photos'?'photos':tab==='status'?'statuses':tab==='repost'?'reposts':tab==='polls'?'polls':'saved posts';
      if(!tabPosts.length){
        fc.innerHTML = '<div class="opu-empty" style="padding:40px;text-align:center;color:var(--text3);">No '+emptyLabel+' yet 🌸</div>';
        return;
      }
      fc.innerHTML = '';
      if(opuLayout==='list' || tab==='status' || tab==='repost' || tab==='polls'){
        ...
        tabPosts.forEach(function(p){
          ...
          if(p.media && p.media.length){
            // Mixed media post (photo + video, any order) — same logic as the owner's own grid
            var _gfirst=p.media[0];
            var _mVid=p.media.filter(function(m){return m.type==='video';}).length;
            var _mImg=p.media.length-_mVid;
            var _mixedIcon=(_mVid&&_mImg)?('<div class="grid-multi-icon" style="width:auto;padding:2px 6px;font-size:10px;color:#fff;white-space:nowrap;">📷'+_mImg+' 🎥'+_mVid+'</div>')
              :(p.media.length>1?'<div class="grid-multi-icon"><svg width="12" height="12" fill="none" stroke="white" stroke-width="2" viewBox="0 0 24 24"><rect x="2" y="7" width="15" height="15" rx="2"/><path d="M17 2H22V17"/></svg></div>':'');
            if(_gfirst.type==='video'){
              item.innerHTML='<video src="'+esc(_gfirst.url)+'" preload="metadata" muted playsinline style="width:100%;height:100%;object-fit:cover;display:block;pointer-events:none;"></video>'
                +_mixedIcon+'<div class="grid-overlay"><svg width="18" height="18" fill="white" viewBox="0 0 24 24"><polygon points="5 3 19 12 5 21 5 3"/></svg><span>❤️ '+p.likes+'</span><span>💬 '+p.comments.length+'</span></div>';
              var _gv=item.querySelector('video');
              if(_gv) _gv.addEventListener('loadedmetadata',function(){_gv.currentTime=Math.min(1,_gv.duration*0.1||1);});
            } else {
              item.innerHTML='<img src="'+esc(_gfirst.url)+'" alt="">'+_mixedIcon+'<div class="grid-overlay"><span>❤️ '+p.likes+'</span><span>💬 '+p.comments.length+'</span></div>';
            }
          } else if(p.isVideo && p.videoUrl){
            // Legacy single-video post (pre-dating mixed media)
            item.innerHTML='<video src="'+esc(p.videoUrl)+'" preload="metadata" muted playsinline style="width:100%;height:100%;object-fit:cover;display:block;pointer-events:none;"></video>'
              +'<div class="grid-overlay"><svg width="18" height="18" fill="white" viewBox="0 0 24 24"><polygon points="5 3 19 12 5 21 5 3"/></svg><span>❤️ '+p.likes+'</span><span>💬 '+p.comments.length+'</span></div>';
            var _gv2=item.querySelector('video');
            if(_gv2) _gv2.addEventListener('loadedmetadata',function(){_gv2.currentTime=Math.min(1,_gv2.duration*0.1||1);});
          } else if(imgs[0]){
            item.innerHTML='<img src="'+imgs[0]+'" alt=""><div class="grid-overlay">'+(imgs.length>1?'<div class="grid-multi-icon"><svg width="12" height="12" fill="none" stroke="white" stroke-width="2" viewBox="0 0 24 24"><rect x="2" y="7" width="15" height="15" rx="2"/><path d="M17 2H22V17"/></svg></div>':'')+'<span>❤️ '+p.likes+'</span><span>💬 '+p.comments.length+'</span></div>';
          } else {
            item.innerHTML = '<div class="grid-item-placeholder" style="font-size:24px;">'+(p.isStatus?'💬':'🖼️')+'</div>';
          }
...
function toggleFollowUserOpu(uid, name){
  ...
  if(idx > -1){
    ...
    db.collection('profiles').doc(uid).update({followersCount:firebase.firestore.FieldValue.increment(1)}).catch(()=>{});
    sendNotification(uid,'follow',me,'');checkAchievements();
  }
  saveProfileToFirestore();
...
async function openDMWith(otherUid,otherName){
  ...
  function dmPresenceLabel(uid){
    ...
    if(status==='online') return '<span style="color:#22c55e;">● Active now</span>';
    if(status==='away') return '<span style="color:#f59e0b;">🌙 Away</span>';
    const p = onlineFriends[uid];
    if(p && p.lastActive){
      const ms = p.lastActive.toMillis ? p.lastActive.toMillis() : (p.lastActive.seconds ? p.lastActive.seconds*1000 : 0);
  ...
  chatArea.innerHTML=`
    <div class="chat-header">
      ...
      <div class="post-avatar" style="background:${otherProf.color||'var(--pink)'};width:36px;height:36px;font-size:13px;cursor:pointer;position:relative;" onclick="viewProfile(_g(${_r(otherUid)}))">
        ...
        ${getPresenceStatus(otherUid)==='online'?'<div style="position:absolute;bottom:1px;right:1px;width:10px;height:10px;background:#22c55e;border-radius:50%;border:2px solid var(--card);"></div>':''}
      </div>
      <div style="cursor:pointer;flex:1;" onclick="viewProfile(_g(${_r(otherUid)}))">
        <div style="font-size:14px;font-weight:600;display:flex;align-items:center;gap:4px;">${esc(otherProf.name||'User')}${verifiedBadgeHTML(otherProf,13)}</div>
    ...
    .onSnapshot(async function(snap){
      ...
      sorted.forEach(function(d){
        ...
        if(m.mediaUrl){
          ...
          var _sp = posts.find(function(x){ return String(x.id)===String(m.postId); });
          if(_sp){
            var _spImg = (_sp.images&&_sp.images[0])||_sp.image||'';
            var _pollEmoji = '📊';
            var _spText = _sp.isPoll ? (_pollEmoji+' '+(_sp.pollQuestion||'Poll')) : ((_sp.caption||(_sp.isStatus?'Status post':'Photo post')).slice(0,100));
            var _spHandle = (_sp.user&&_sp.user.handle)||'user';
            var _spPid = String(_sp.id);
            _dmBc = '<div onclick="openLightbox(\''+_spPid+'\')" style="cursor:pointer;border:1.5px solid var(--border);border-radius:12px;overflow:hidden;max-width:230px;background:var(--bg2);">'
              +(_spImg?'<img src="'+esc(_spImg)+'" style="width:100%;max-height:140px;object-fit:cover;display:block;">':'')
              +'<div style="padding:8px 10px 2px;font-size:12.5px;color:var(--text);font-weight:500;line-height:1.4;">'+esc(_spText)+'</div>'
              +'<div style="padding:2px 10px 8px;font-size:11px;color:var(--pink);">@'+esc(_spHandle)+' · tap to view →</div>'
              +'</div>';
            _dmBs = 'background:transparent;padding:0;border:none;box-shadow:none;';
          } else {
            _dmBc = esc(m.sharedPreview||'Shared a post');
          }
        } else {
          _dmBc = esc(m.text||'');
        }
        div.innerHTML = '<div class="chat-bubble" style="'+_dmBs+'">'+_dmBc+'</div>'
          + (timeStr ? '<div class="chat-time" style="font-size:10px;color:var(--text3);margin-top:2px;'+(mine?'text-align:right;':'')+'">'+timeStr+(m.edited?' · <span style="font-style:italic;">edited</span>':'')+'</div>' : '');
        // Double-click to edit — only for my own plain text messages (not media/stickers/shared posts)
        var _canEditMsg = mine && m.text && !m.mediaUrl && !m.stickerUrl && !m.postId && !m.isEmoji && m.id;
        if(_canEditMsg){
          div.querySelector('.chat-bubble').addEventListener('dblclick', function(){
            _startEditDMMessage(convoId, m.id, m.text, div);
          });
        }
...
// Inline edit for my own DM text messages (double-click bubble to trigger)
function _startEditDMMessage(convoId, msgId, currentText, div){
  ...
  bubble.innerHTML = '<textarea style="width:100%;min-width:180px;background:transparent;border:none;outline:none;color:inherit;font-family:inherit;font-size:inherit;resize:none;" rows="2"></textarea>'
    + '<div style="display:flex;gap:6px;justify-content:flex-end;margin-top:4px;">'
    + '<button class="edit-msg-cancel" style="background:none;border:none;font-size:11px;color:var(--text3);cursor:pointer;">Cancel</button>'
    + '<button class="edit-msg-save" style="background:none;border:none;font-size:11px;color:var(--pink);font-weight:600;cursor:pointer;">Save</button>'
    + '</div>';
  var ta = bubble.querySelector('textarea');
  ta.value = currentText;
  ta.focus();
  ta.setSelectionRange(currentText.length, currentText.length);
  bubble.querySelector('.edit-msg-cancel').onclick = function(){ bubble.innerHTML = original; };
  bubble.querySelector('.edit-msg-save').onclick = async function(){
    var newText = ta.value.trim();
    if(!newText){ toast('Message cannot be empty'); return; }
    try{
      await db.collection('dms').doc(convoId).collection('messages').doc(msgId).update({
        text: newText, edited: true, editedAt: firebase.firestore.FieldValue.serverTimestamp()
      });
      toast('Message edited');
      // The live snapshot listener will re-render with the updated text + "edited" tag automatically
    }catch(e){ toast('Could not edit message'); bubble.innerHTML = original; }
  };
}
...
async function sendDM(convoId){
  ...
  sendMentionNotifications(txt, convoId);
}
      ...
      div.innerHTML=
        ...
        +'<div class="msg-conv-info" style="flex:1;min-width:0;">'
          ...
          +'<div style="display:flex;justify-content:space-between;align-items:center;">'
            ...
            +(function(){
              var status=getPresenceStatus(otherUid);
              if(status==='online') return '<div style="font-size:10px;color:#22c55e;flex-shrink:0;white-space:nowrap;margin-left:6px;">● active now</div>';
              if(status==='away') return '<div style="font-size:10px;color:#f59e0b;flex-shrink:0;white-space:nowrap;margin-left:6px;">🌙 away</div>';
              var pres=onlineFriends[otherUid];
              if(pres&&pres.lastActive){var lsMs=pres.lastActive.toMillis?pres.lastActive.toMillis():(pres.lastActive.seconds?pres.lastActive.seconds*1000:0);if(lsMs)return '<div style="font-size:10px;color:var(--text3);flex-shrink:0;white-space:nowrap;margin-left:6px;">'+timeAgo(lsMs)+'</div>';}
              return '';
            })()
          +'</div>'
        +'</div>';
...
async function openGroupConvo(gcId, gcData){
  ...
  dmUnsubscribe = db.collection('groups').doc(gcId).collection('messages')
    .onSnapshot(async function(snap){
      ...
      sorted.forEach(function(d){
        var m=d.data();
        if(!m.text&&!m.mediaUrl&&!m.postId) return;
        var mine=m.from===me.uid;
        var tsMs=m.ts?(m.ts.toMillis?m.ts.toMillis():m.ts.seconds?m.ts.seconds*1000:0):0;
        ...
        var textHTML = '';
        if(m.postId){
          var _gcSp = posts.find(function(x){ return String(x.id)===String(m.postId); });
          if(_gcSp){
            var _gcImg = (_gcSp.images&&_gcSp.images[0])||_gcSp.image||'';
            var _gcPollE = '📊';
            var _gcText = _gcSp.isPoll ? (_gcPollE+' '+(_gcSp.pollQuestion||'Poll')) : ((_gcSp.caption||(_gcSp.isStatus?'Status post':'Photo post')).slice(0,100));
            var _gcHandle = (_gcSp.user&&_gcSp.user.handle)||'user';
            var _gcPid = String(_gcSp.id);
            textHTML = '<div onclick="openLightbox(\''+_gcPid+'\')" style="cursor:pointer;border:1.5px solid var(--border);border-radius:12px;overflow:hidden;max-width:230px;background:var(--bg3);">';
            if(_gcImg) textHTML += '<img src="'+esc(_gcImg)+'" style="width:100%;max-height:130px;object-fit:cover;display:block;">';
            textHTML += '<div style="padding:7px 10px 2px;font-size:12.5px;color:var(--text);font-weight:500;line-height:1.4;">'+esc(_gcText)+'</div>';
            textHTML += '<div style="padding:2px 10px 8px;font-size:11px;color:var(--pink);">@'+esc(_gcHandle)+' · tap to view →</div></div>';
          } else {
            textHTML = esc(m.sharedPreview||'Shared a post');
          }
        } else if(m.text){
          textHTML = esc(m.text).replace(/@(\w+)/g, function(match, handle){
...
async function sendGroupMsg(gcId){
  ...
  try{
    ...
    sendMentionNotifications(txt, gcId);
...
async function handleGCMentionInput(inp, gcId){
  ...
  filtered.forEach(function(u){
    ...
    item.innerHTML='<div style="width:28px;height:28px;border-radius:50%;background:'+(u.color||'var(--pink)')+';display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:white;overflow:hidden;flex-shrink:0;">'
      +(u.avatar?'<img src="'+esc(u.avatar)+'" style="width:100%;height:100%;object-fit:cover;">':esc(u.initial||'?'))
    +'</div>'
    +'<div><div style="font-size:13px;font-weight:600;color:var(--text);display:flex;align-items:center;gap:3px;">'+esc(u.name||u.handle)+verifiedBadgeHTML(u,11)+'</div><div style="font-size:11px;color:var(--text3);">@'+esc(u.handle)+'</div></div>';
...
async function publishStory(){
  ...
  if(_storyType==='photo'&&_storyFileData){
    try{
      var url=await uploadToStorage(_storyFileData,'kez-media/stories/'+me.uid+'/'+Date.now());
      storyData.mediaUrl=url;
      storyData.cloudinaryFolder='kez-media/stories/'+me.uid;
    }catch(e){ storyData.mediaUrl=_storyFileData; }
  }
  var storyRef=await db.collection('stories').add(_stripUndefined(storyData));
  // Mentions in story captions/text — this notification path didn't exist before
  var storyMentionText = (photoCaption||'') + ' ' + (storyData.text||'');
  if(storyMentionText.trim()) sendMentionNotifications(storyMentionText, storyRef.id);
  // Log to activityLog for admin Cloudinary tracking
  db.collection('activityLog').add({
    type:'story', uid:me.uid, handle:me.handle||'',
    storyId:storyRef.id, mediaUrl:storyData.mediaUrl||'',
    cloudinaryFolder:storyData.cloudinaryFolder||'',
    ts:firebase.firestore.FieldValue.serverTimestamp()
  }).catch(function(){});
  // Reset caption input
  ...
  loadStories();
  me.storiesPostedCount = (me.storiesPostedCount||0)+1;
  db.collection('profiles').doc(me.uid).set({storiesPostedCount: firebase.firestore.FieldValue.increment(1)},{merge:true}).catch(function(){});
  checkAchievements();
}
...
// ── STORY REPLY → DM ─────────────────────────────────
async function replyToStoryViaDM(storyId, text){
  text=(text||'').trim(); if(!text) return;
  var s=_svStories.find(function(x){return x.id===storyId;})||_svStories[_svIdx];
  if(!s||!me.uid) return;
  var convoId=[me.uid,s.uid].sort().join('_');
  var msg={
    from:me.uid, fromName:me.name, fromHandle:me.handle,
    text:text, storyReply:true, storyId:s.id,
    storyThumb:s.mediaUrl||s.imageUrl||null,
    ts:firebase.firestore.FieldValue.serverTimestamp()
  };
  ...
  await db.collection('dms').doc(convoId).update({
    ...
    lastMsg:'Replied to your story: "'+text.slice(0,40)+'"',
    lastTs:firebase.firestore.FieldValue.serverTimestamp(),
    ['unread_'+s.uid]:firebase.firestore.FieldValue.increment(1)
  }).catch(function(){});
  sendNotification(s.uid,'story_reply',me,me.name+' replied: "'+text.slice(0,40)+'"',s.id);
  toast('Reply sent! 💌');
}

// ── DELETE STORY (archives first so media stays traceable) ────
async function deleteStory(storyId){
  if(!confirm('Delete this story?')) return;
  var s=_svStories.find(function(x){return x.id===storyId;});
  if(!s||s.uid!==me.uid){ toast('Not your story'); return; }
  // Archive before delete
  try{
    await db.collection('storyArchive').add({
      ...s, storyId:s.id,
      archivedAt:firebase.firestore.FieldValue.serverTimestamp(),
      archiveReason:'user-deleted'
    });
  }catch(e){}
  await db.collection('stories').doc(storyId).delete().catch(function(){});
  _svStories=_svStories.filter(function(x){return x.id!==storyId;});
  if(!_svStories.length){closeStoryViewer();}
  else{storyNav(0);}
  loadStories();
  toast('Story deleted');
}
...
async function archiveMyExpiredStories(){
  if(!me.uid) return;
  try{
    var now=new Date().toISOString();
    var snap=await db.collection('stories').where('uid','==',me.uid).where('expiresAt','<',now).get();
    snap.docs.forEach(async function(d){
      var s={id:d.id,...d.data()};
      await db.collection('storyArchive').add({
        ...s, storyId:s.id,
        archivedAt:firebase.firestore.FieldValue.serverTimestamp(),
      ...
      await d.ref.delete().catch(function(){});
    });
  }catch(e){}
}
...
async function openStoryArchive(){
  ...
  _hlArchiveAddMode=false; // default off; the highlight editor's "Add Stories" button flips this true right after calling us
  ...
  // Archive = your still-active stories (not expired yet) PLUS everything already moved to storyArchive.
  // This is the fix: the old version only ever read the live "stories" collection, so once a story
  // actually got archived (and deleted from "stories"), it silently vanished from this screen.
  var liveSnap = await db.collection('stories').where('uid','==',me.uid).get().catch(()=>({docs:[]}));
  var archSnap = await db.collection('storyArchive').where('uid','==',me.uid).get().catch(()=>({docs:[]}));
  var seenStoryIds = new Set();
  var stories = [];
  ...
  archSnap.docs.forEach(function(d){
    var s={id:d.id,...d.data()};
    var origId = s.storyId || s.id;
    if(seenStoryIds.has(origId)) return; // avoid double-listing a story present in both collections briefly
    seenStoryIds.add(origId);
    s.id = origId; // keep it addressable by its original story id for highlight/viewer purposes
    s._archivedDoc = d.id;
    stories.push(s);
  });
  stories.sort(function(a,b){
    var ta=a.createdAt?(a.createdAt.toMillis?a.createdAt.toMillis():(a.createdAt.seconds?a.createdAt.seconds*1000:0)):0;
    var tb=b.createdAt?(b.createdAt.toMillis?b.createdAt.toMillis():(b.createdAt.seconds?b.createdAt.seconds*1000:0)):0;
    return tb-ta;
  });
  ...
  stories.forEach(function(s){
    var expired = (s.expiresAt && s.expiresAt<now) || !!s._archivedDoc;
    // Date separator (spec: "Newest first, Date separators")
    var d0 = s.createdAt?(s.createdAt.toMillis?s.createdAt.toMillis():(s.createdAt.seconds?s.createdAt.seconds*1000:Date.now())):Date.now();
    var dateLabel = new Date(d0).toLocaleDateString(undefined,{month:'short',day:'numeric',year:'numeric'});
    if(dateLabel!==lastDateLabel){
      lastDateLabel=dateLabel;
      var sep=document.createElement('div');
      sep.style.cssText='grid-column:1/-1;font-size:11.5px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.04em;padding:10px 2px 2px;';
      sep.textContent=dateLabel;
      grid.appendChild(sep);
    }
    var item=document.createElement('div');item.className='sa-item';item.style.position='relative';
    if(s.type==='text'){
      item.style.background=s.bgColor||'linear-gradient(135deg,#e2688a,#f0a0b8)';
      item.innerHTML='<div class="sa-item-text">'+esc(s.text)+'</div>';
    } else if(s.mediaUrl){
      item.innerHTML = s.type==='video'
        ? '<video src="'+esc(s.mediaUrl)+'" muted preload="metadata"></video><div class="sa-type-icon">🎬</div>'
        : '<img src="'+esc(s.mediaUrl)+'" alt=""><div class="sa-type-icon">📷</div>';
    }
    item.innerHTML+='<div class="sa-item-date">'+(expired?'Archived · ':'Active · ')+timeAgo(s.createdAt)+'</div>';
    // Add to Highlight — available for ANY story with media, active or archived (this was the other bug:
    // it used to only offer this for non-expired stories, which defeats the point of highlights)
    if(s.mediaUrl){
      var hlBtn=document.createElement('button');
      hlBtn.innerHTML='📌';
      hlBtn.title='Add to Highlight';
      hlBtn.style.cssText='position:absolute;top:5px;right:5px;background:rgba(0,0,0,.55);border:none;color:white;border-radius:8px;padding:3px 7px;font-size:11px;cursor:pointer;z-index:2;';
      hlBtn.onclick=function(e){
        e.stopPropagation();
        var storyPayload={mediaUrl:s.mediaUrl||'',mediaType:s.type==='video'?'video':'image',caption:s.caption||s.textOverlay||'',createdAt:d0};
        if(_hlArchiveAddMode && _editingHighlightId){
          _hlArchiveAddMode=false;
          addStoryToHighlight(storyPayload, _editingHighlightId).then(function(){
            closeStoryArchive();
            openHighlightEditor(_editingHighlightId);
          });
        } else {
          openAddToHighlightPicker(storyPayload);
        }
      };
      item.appendChild(hlBtn);
    }
    item.onclick=function(){
      if(!expired){closeStoryArchive();openStoryViewer([s],0);}
      else{ closeStoryArchive(); openStoryViewer([s],0); } // archived stories are still viewable — just no longer public/live
    };
    grid.appendChild(item);
  });
}
...
function openEditPost(id){
  ...
  // Video and YouTube: show video/URL edit section
...
function epmHandleNewVideo(input){
  var file = input.files[0];
  if(!file) return;
  var reader = new FileReader();
  reader.onload = function(e){
    _editNewVideo = e.target.result;
    // Update preview with new video
    var preview = document.getElementById('epmPreview');
    preview.innerHTML = '<video src="'+_editNewVideo+'" style="width:100%;max-height:200px;display:block;" controls playsinline></video>';
    toast('New video loaded - will be uploaded when you save');
  };
  reader.readAsDataURL(file);
}
...
async function saveEditPost(){
  ...
  try {
    ...
    if(newVideoUrl && newVideoUrl.value && newVideoUrl.value.trim() && (p.isVideo || p.isYoutube)){
      ...
      if(url.includes('youtube.com') || url.includes('youtu.be')){
        ...
        if(ytId){
          updates.ytUrl = url;
          updates.ytId = ytId;
          updates.ytThumb = 'https://i.ytimg.com/vi/'+ytId+'/maxresdefault.jpg';
          updates.isYoutube = true;
          updates.isVideo = false;
          p.ytUrl = url;
          p.ytId = ytId;
          p.ytThumb = updates.ytThumb;
          p.isYoutube = true;
          p.isVideo = false;
        }
      } else {
    ...
    if(typeof _editNewVideo !== 'undefined' && _editNewVideo){
      try{
        var uploadedVideo = await uploadToStorage(_editNewVideo, 'posts/'+me.uid+'/video_'+Date.now());
        if(uploadedVideo){
          updates.videoUrl = uploadedVideo;
          updates.isVideo = true;
          updates.isYoutube = false;
          p.videoUrl = uploadedVideo;
          p.isVideo = true;
          p.isYoutube = false;
        }
      }catch(e){ toast('Video upload failed — caption still saved'); }
    }
...
function showMentionDropdown(inp,users,atIdx,pid){
  ...
  users.forEach(function(u){
    ...
    item.innerHTML='<div class="mention-av" style="background:'+esc(u.color||'var(--pink)')+'">'+avImg+'</div><div><strong style="display:inline-flex;align-items:center;gap:3px;">'+esc(u.name||u.handle)+verifiedBadgeHTML(u,12)+'</strong><div style="font-size:11px;color:var(--text3);">@'+esc(u.handle)+'</div></div>';
...
function searchPageAll(q){
  ...
  searchPageUsers(q); // Find People is users-only now — posts/tags discovery moved to Explore
}
...
// ── VERIFIED BADGE (single source of truth) ───────────
// Reused everywhere a username/handle is rendered: feed, comments, replies,
// notifications, search, profile previews, friends/followers/following,
// messages, chat header, admin dashboard.
// Rule: verified === true in Firestore → show badge. Otherwise → show nothing.
// (Admin accounts also get it automatically, matching prior behavior — no hardcoded usernames.)
let _vbCounter = 0;
function verifiedBadgeHTML(u, size){
  if(!u) return '';
  var isVerified = !!(u.verified) || !!(u.isAdmin) ||
                   !!(window._KEZ_ADMIN_UID && window._KEZ_ADMIN_UID === (u.uid||u.id)) ||
                   !!(u.email && ['keshamaebangcoyo9@gmail.com'].includes(u.email));
  if(!isVerified) return '';
  var s=size||14, gid='vb'+(_vbCounter++);
  return '<svg class="verified-badge-icon" width="'+s+'" height="'+s+'" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="flex-shrink:0;vertical-align:middle;display:inline-block;" title="Verified"><defs><radialGradient id="'+gid+'bg" cx="35%" cy="25%" r="70%"><stop offset="0%" stop-color="#fde8f2"/><stop offset="30%" stop-color="#f0a0b8"/><stop offset="65%" stop-color="#e2688a"/><stop offset="100%" stop-color="#be185d"/></radialGradient><radialGradient id="'+gid+'sh" cx="30%" cy="20%" r="50%"><stop offset="0%" stop-color="white" stop-opacity="0.7"/><stop offset="100%" stop-color="white" stop-opacity="0"/></radialGradient></defs><path d="M12 2l2.4 3.2 3.9-.8-1 3.8 3.2 2.3-3.2 2.3 1 3.8-3.9-.8L12 22l-2.4-3.2-3.9.8 1-3.8L3.5 13.5l3.2-2.3-1-3.8 3.9.8z" fill="url(#'+gid+'bg)"/><path d="M12 2l2.4 3.2 3.9-.8-1 3.8 3.2 2.3-3.2 2.3 1 3.8-3.9-.8L12 22l-2.4-3.2-3.9.8 1-3.8L3.5 13.5l3.2-2.3-1-3.8 3.9.8z" fill="url(#'+gid+'sh)"/><polyline points="8.5 12.5 10.8 14.8 15.5 10" stroke="white" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" fill="none"/></svg>';
}
// Looks up the live/current user object for a uid (preferring allUsers cache), falls back to an embedded user snapshot
function _liveUserFor(uid, fallback){
  return (uid && typeof allUsers!=='undefined' && allUsers[uid]) ? allUsers[uid] : (fallback||{});
}
...
// After 3.5s: hide splash and show whatever is ready
// (was 2.2s — increased for deployed environments where Firebase cold-starts slower)
setTimeout(function(){
  _hideSplash();
  if(!_appReady){
...
async function runAppInit(user){
  // Guard against double-fire (onAuthStateChanged can fire twice on redirect auth)
  if(_appReady) return;
  try {
    // STEP 1: Set uid/email immediately so loadProfile can fetch the doc
    me = { uid: user.uid, email: user.email };
    // Firebase Auth's own account-creation timestamp — this is the authoritative "joined" date,
    // set once by Firebase itself at signup and never editable/losable, unlike a Firestore field
    // that could theoretically go missing. Always available on the auth user object directly.
    if(user.metadata && user.metadata.creationTime){
      me.authJoinedAt = new Date(user.metadata.creationTime).getTime();
    }

    // STEP 2: Load saved profile from Firestore FIRST before anything else
    await loadProfile(user.uid);
    // Backfill this authoritative join date into the profile doc if it's missing or drifted,
    // so admin views / other places reading the Firestore doc directly stay accurate too.
    if(me.authJoinedAt && me.joinedAt!==me.authJoinedAt){
      me.joinedAt = me.authJoinedAt;
      db.collection('profiles').doc(user.uid).set({joinedAt: me.authJoinedAt},{merge:true}).catch(function(){});
    }
...
auth.onAuthStateChanged(function(user){
  if(user){
    ...
    _appReady = false;
    // NOTE: do NOT reset _splashGone here — resetting it causes the splash
    // to re-trigger on signout/signin cycles within the same session.
...
let onlineFriends = {}; // uid -> presence doc data
var _lastInteractionAt = Date.now();
var _presenceRef = null;
var HEARTBEAT_MS = 30000;      // how often we ping while the tab is visible
var AWAY_THRESHOLD_MS = 3*60*1000;   // no interaction for 3 min (within spec's 2-5 min range) → away
var OFFLINE_STALE_MS = 90*1000;      // no heartbeat for 90s (3 missed beats) → treat as offline, even if their
                                      // tab never fired beforeunload (crash, force-close, lost network, etc.)

// Real activity = mouse, keyboard, scroll, touch, click — not just "tab is open"
function _markActivity(){ _lastInteractionAt = Date.now(); }
['mousemove','keydown','scroll','touchstart','click','input'].forEach(function(evt){
...
function startPresence(){
  if(!me.uid) return;
  _presenceRef = db.collection('presence').doc(me.uid);
  _sessionStartedAt = Date.now();
  _sendHeartbeat(); // immediate first beat
  presenceInterval = setInterval(function(){
    if(!document.hidden) _sendHeartbeat();
  }, HEARTBEAT_MS);
  // Page Visibility API: pause heartbeats while hidden/minimized, resume immediately when back.
  // We deliberately do NOT write an explicit "offline" flag on hide/unload anymore — that write is
  // unreliable (mobile browsers and crashed tabs often skip it entirely). Instead, other clients
  // derive online/away/offline purely from how stale lastActive/lastInteractionAt are. This is
  // self-healing: a crashed tab just goes quiet and naturally ages into "offline" within ~90s.
...
function _sendHeartbeat(){
  ...
  _presenceRef.set({
    ...
    name: me.name||me.handle, handle: me.handle, color: me.color, initial: me.initial, avatar: me.avatar||null,
    userAgent: navigator.userAgent.slice(0,140),
    currentPage: (document.title||'').slice(0,60),
...
function watchPresence(){
  ...
  db.collection('presence').onSnapshot(function(snap){
    ...
    document.querySelectorAll('[data-presence-uid]').forEach(function(el){
      ...
      el.style.display = (getPresenceStatus(uid)==='online') ? '' : 'none';
    });
  }, function(){});
}

// Derives status purely from staleness, never from a stored boolean — this is what makes it
// self-healing against tabs that closed uncleanly (crash, force-quit, killed by the OS, etc.)
function getPresenceStatus(uid){
  ...
  if(Date.now() - lastActiveMs > OFFLINE_STALE_MS) return 'offline'; // heartbeat stopped → they're gone
  const lastInteractionMs = p.lastInteractionAt || lastActiveMs;
  if(Date.now() - lastInteractionMs > AWAY_THRESHOLD_MS) return 'away'; // tab open, heartbeat alive, but idle
  return 'online';
}
...
function searchPageUsers(q){
  ...
  filtered.slice(0,40).forEach(function(u){
    ...
    card.innerHTML =
      ...
      +'</div>'
      +'<div class="people-card-name" style="display:inline-flex;align-items:center;gap:3px;justify-content:center;">'+esc(u.name||u.handle||'User')+verifiedBadgeHTML(u,12)+'</div>'
...
async function refreshAdminDashboard(){
  ...
  const onlineCount = Object.keys(onlineFriends).filter(function(uid){ return getPresenceStatus(uid)==='online'; }).length;
...
async function sendPostViaGC(postId, group){
  ...
  var previewLabel = p
    ? (p.isPoll ? '📊 Poll: '+(p.pollQuestion||'').slice(0,60) : '📎 Post by @'+(p.user&&p.user.handle||'user'))
    : '📎 Shared post';
  try{
    await db.collection('groups').doc(group.id).collection('messages').add({
      from:me.uid, fromName:me.name, fromHandle:me.handle,
      text:'', postId:postId, sharedPreview:previewLabel,
      ts:firebase.firestore.FieldValue.serverTimestamp()
    });
    await db.collection('groups').doc(group.id).update({
      lastMsg:previewLabel, lastTs:firebase.firestore.FieldValue.serverTimestamp()
    });
    var members=(group.members||[]).filter(function(u){return u!==me.uid;});
    members.forEach(function(uid){ sendNotification(uid,'message',me,me.name+' shared a post'); });
...
async function sendPostViaDM(postId, toUser){
  ...
  setTimeout(async function(){
    ...
    var _sharedP2 = posts.find(function(x){ return String(x.id)===String(postId); });
    var _previewLabel2 = _sharedP2
      ? (_sharedP2.isPoll ? '📊 Poll: '+(_sharedP2.pollQuestion||'').slice(0,60) : '📎 Post by @'+(_sharedP2.user&&_sharedP2.user.handle||'user'))
      : '📎 Shared post';
    var msg = {
      from: me.uid,
      fromName: me.name,
...
function renderActiveUsersInMessages(){
  ...
  if(!bar) return;
  var online = Object.entries(onlineFriends).filter(function(e){ return getPresenceStatus(e[0])==='online'; });
...
function renderFeedPaged(){
  ...
  if(!feed)return;
  var visible=posts.filter(function(p){return !p._community&&!p.hidden&&!_locallyDeletedIds.has(String(p.id));}).filter(function(p){return !isBlocked(p.uid);}).slice(0,_postsPage*POSTS_PER_PAGE);
  feed.innerHTML='';
  ...
  var total=posts.filter(function(p){return !p._community&&!p.hidden&&!_locallyDeletedIds.has(String(p.id));}).length;
...
function updateVerifiedBadge(){
  ...
  if(vb){
    // Admin only
    vb.style.display=isAdmin()?'inline-flex':'none';
  }
}
...
function _buildAddHighlightBtn(){
...
function renderHighlights(){
  ...
  if(!wrap)return;
  wrap.innerHTML='';
  // Own profile: always show the row (so the "+ New" button is reachable even with zero highlights)
  if(!_highlights.length){
    var emptyRow=document.createElement('div');
    emptyRow.style.cssText='display:flex;gap:14px;overflow-x:auto;padding:12px 16px 4px;scrollbar-width:none;';
    emptyRow.appendChild(_buildAddHighlightBtn());
  ...
  row.appendChild(_buildAddHighlightBtn());
  // Existing highlights
  _highlights.forEach(function(h){
    var item=document.createElement('div');
    item.style.cssText='display:flex;flex-direction:column;align-items:center;gap:5px;cursor:pointer;flex-shrink:0;position:relative;';
    var cover=h.coverUrl||h.stories&&h.stories[0]&&h.stories[0].mediaUrl||'';
    item.innerHTML='<div style="position:relative;width:60px;height:60px;">'
      +'<div style="width:60px;height:60px;border-radius:50%;background:var(--bg3);overflow:hidden;border:2.5px solid var(--border);">'
      ...
      +'</div>'
      +'<button class="hl-edit-btn" title="Edit highlight" style="position:absolute;bottom:0px;right:0px;width:14px;height:14px;border-radius:50%;background:var(--pink);border:1px solid var(--card);color:white;font-size:7px;display:flex;align-items:center;justify-content:center;cursor:pointer;padding:0;">✎</button>'
      +'</div>'
    ...
    item.querySelector('.hl-edit-btn').onclick=function(e){e.stopPropagation();openHighlightEditor(h.id);};
...
function openCreateHighlight(pendingStory){
  ...
  modal.innerHTML='<div style="background:var(--card);border-radius:22px 22px 0 0;padding:24px 24px max(90px,env(safe-area-inset-bottom));width:100%;max-width:480px;max-height:80dvh;overflow-y:auto;">'
  ...
  window._pendingHighlightStory = pendingStory || null;
...
async function saveHighlight(){
  ...
  var pending=window._pendingHighlightStory;
  var initialStories = pending ? [{
    mediaUrl:pending.mediaUrl||'', mediaType:pending.mediaType||'image',
    caption:pending.caption||'', addedAt:Date.now(), storyCreatedAt:pending.createdAt||Date.now()
  }] : [];
  var hl={id:hlId,title,stories:initialStories,coverUrl:pending&&pending.mediaUrl||'',createdAt:firebase.firestore.FieldValue.serverTimestamp()};
  try{
    ...
    window._pendingHighlightStory=null;
    toast(pending?'Highlight created and story added! 📌':'Highlight created! Add stories to it from your archive.');
  }catch(e){toast('Could not create highlight');}
}

// Shows "Create New" or pick an existing highlight to add this story to
function openAddToHighlightPicker(story){
  closeStoryArchive();
  var modal=document.createElement('div');
  modal.id='hlPickerModal';
  modal.style.cssText='position:fixed;inset:0;z-index:5100;background:rgba(0,0,0,.7);backdrop-filter:blur(6px);display:flex;align-items:flex-end;justify-content:center;';
  var existingHTML = _highlights.map(function(h){
    var cover=h.coverUrl||(h.stories&&h.stories[0]&&h.stories[0].mediaUrl)||'';
    return '<div class="hl-pick-item" style="display:flex;align-items:center;gap:12px;padding:10px 4px;cursor:pointer;" data-hlid="'+esc(h.id)+'">'
      +'<div style="width:44px;height:44px;border-radius:50%;overflow:hidden;background:var(--bg3);flex-shrink:0;">'
      +(cover?'<img src="'+esc(cover)+'" style="width:100%;height:100%;object-fit:cover;">':'<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;">📌</div>')
      +'</div><div style="font-size:14px;font-weight:600;">'+esc(h.title||'Highlight')+'</div>'
      +'<div style="margin-left:auto;font-size:11.5px;color:var(--text3);">'+((h.stories||[]).length)+' stories</div>'
      +'</div>';
  }).join('');
  modal.innerHTML='<div style="background:var(--card);border-radius:22px 22px 0 0;padding:20px 20px max(90px,env(safe-area-inset-bottom));width:100%;max-width:480px;max-height:75dvh;overflow-y:auto;">'
    +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">'
    +'<div style="font-size:16px;font-weight:700;">Add to Highlight</div>'
    +'<button onclick="document.getElementById(\'hlPickerModal\').remove()" style="background:none;border:none;font-size:22px;cursor:pointer;color:var(--text3);">×</button>'
    +'</div>'
    +'<div class="hl-pick-item" id="hlPickNew" style="display:flex;align-items:center;gap:12px;padding:10px 4px;cursor:pointer;border-bottom:1px solid var(--border);margin-bottom:6px;">'
    +'<div style="width:44px;height:44px;border-radius:50%;border:2px dashed var(--border);display:flex;align-items:center;justify-content:center;flex-shrink:0;">'
    +'<svg width="16" height="16" fill="none" stroke="var(--pink)" stroke-width="2" viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>'
    +'<div style="font-size:14px;font-weight:600;color:var(--pink);">Create New Highlight</div></div>'
    +existingHTML
    +(!_highlights.length?'<div style="font-size:12.5px;color:var(--text3);padding:8px 4px;">You don\'t have any highlights yet — create one above.</div>':'')
    +'</div>';
  document.body.appendChild(modal);
  document.getElementById('hlPickNew').onclick=function(){
    modal.remove();
    openCreateHighlight(story);
  };
  modal.querySelectorAll('.hl-pick-item[data-hlid]').forEach(function(el){
    el.onclick=function(){
      var hlId=el.dataset.hlid;
      modal.remove();
      addStoryToHighlight(story, hlId);
    };
  });
}

async function addStoryToHighlight(story, highlightId){
  if(!highlightId){ openAddToHighlightPicker(story); return; }
  try{
    ...
    stories.push({
      mediaUrl:story.mediaUrl||'', mediaType:story.mediaType||'image',
      caption:story.caption||'', addedAt:Date.now(), storyCreatedAt:story.createdAt||Date.now()
    });
...
// Public highlights viewer for someone else's profile — read-only, no add/edit/delete controls.
// Story Archive itself is never queried here — only the public highlights subcollection.
// ── ACHIEVEMENT SYSTEM (v1.3) — automatic, milestone-based ──
// Separate from the (future) admin-granted manual badge system.
// Firestore: profiles/{uid}/achievements/{achievementId} → {id, unlockedAt}
// ── ACHIEVEMENT ICONS (colorful SVG, no emoji) ────────
// Small reusable shape library + category gradient palette. Each achievement references a shape
// key; the shape is rendered inside a circular gradient badge sized for wherever it's used.
var ACH_ICON_PATHS = {
  login:    '<path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><polyline points="10 17 15 12 10 7"/><line x1="15" y1="12" x2="3" y2="12"/>',
  post:     '<rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/>',
  ...
  star:     '<polygon points="12 2 15.09 8.63 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.63 12 2"/>',
  chatMulti:'<path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8z"/>',
  reply:    '<polyline points="9 17 4 12 9 7"/><path d="M20 18v-2a4 4 0 0 0-4-4H4"/>',
  sparkle:  '<path d="M12 2l1.5 5.5L19 9l-5.5 1.5L12 16l-1.5-5.5L5 9l5.5-1.5z"/><path d="M19 15l.7 2.3L22 18l-2.3.7L19 21l-.7-2.3L16 18l2.3-.7z"/>',
  compass:  '<circle cx="12" cy="12" r="10"/><polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76"/>',
  map:      '<polygon points="1 6 1 22 8 18 16 22 23 18 23 2 16 6 8 2 1 6"/><line x1="8" y1="2" x2="8" y2="18"/><line x1="16" y1="6" x2="16" y2="22"/>',
  flame:    '<path d="M12 2s-2 3-2 6a2 2 0 0 0 4 0c0 2-2 3-2 5a4 4 0 0 0 8 0c0-6-4-7-4-13 0 4-3 5-3 8-1-2-1-4-1-6z"/>',
  shield:   '<path d="M12 2l8 4v6c0 5-3.5 8.5-8 10-4.5-1.5-8-5-8-10V6z"/><polyline points="9 12 11 14 15 10"/>',
  flag:     '<line x1="4" y1="22" x2="4" y2="2"/><path d="M4 4h13l-2.5 4L17 12H4"/>'
};
var ACH_CAT_GRADIENTS = {
  Beginner:  ['#22c55e','#0ea5e9'],
  Creator:   ['#e2688a','#a855f7'],
  Community: ['#f59e0b','#ef4444'],
  Explorer:  ['#0ea5e9','#6366f1'],
  Streak:    ['#f97316','#ef4444'],
  Special:   ['#eab308','#f59e0b']
};
var _achIconGradCounter = 0;
function achIconHTML(shapeKey, cat, size, glow){
  var s = size||40;
  var colors = ACH_CAT_GRADIENTS[cat] || ['#e2688a','#a855f7'];
  var gid = 'achg'+(_achIconGradCounter++);
  var pathData = ACH_ICON_PATHS[shapeKey] || ACH_ICON_PATHS.star;
  var glowStyle = glow ? 'animation:achGlowPulse 2.2s ease-in-out infinite;' : '';
  return '<div style="width:'+s+'px;height:'+s+'px;border-radius:50%;background:linear-gradient(135deg,'+colors[0]+','+colors[1]+');display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 2px 8px '+colors[0]+'55;'+glowStyle+'" data-glow-color="'+colors[0]+'">'
    + '<svg width="'+Math.round(s*0.52)+'" height="'+Math.round(s*0.52)+'" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><defs><linearGradient id="'+gid+'"></linearGradient></defs>'+pathData+'</svg>'
    + '</div>';
}

// Small celebratory confetti burst for the "Achievement Unlocked" moment — lightweight, no external libs
function _achievementConfettiBurst(){
  var colors = ['#e2688a','#a855f7','#22c55e','#f59e0b','#0ea5e9'];
  var container = document.createElement('div');
  container.style.cssText = 'position:fixed;inset:0;pointer-events:none;z-index:9999;overflow:hidden;';
  for(var i=0;i<24;i++){
    var piece = document.createElement('div');
    var color = colors[i%colors.length];
    var startX = 50 + (Math.random()*20-10);
    var driftX = (Math.random()*60-30);
    var fallDuration = 1100 + Math.random()*700;
    piece.style.cssText = 'position:absolute;top:-10px;left:'+startX+'%;width:7px;height:7px;background:'+color+';border-radius:'+(Math.random()>0.5?'50%':'2px')+';opacity:0.95;transform:rotate('+(Math.random()*360)+'deg);';
    piece.animate([
      {transform:'translate(0,0) rotate(0deg)', opacity:1},
      {transform:'translate('+driftX+'px, 70vh) rotate('+(Math.random()>0.5?720:-720)+'deg)', opacity:0.9},
      {transform:'translate('+(driftX*1.4)+'px, 100vh) rotate('+(Math.random()>0.5?1080:-1080)+'deg)', opacity:0}
...
var ACHIEVEMENT_DEFS = [
  // Beginner
  {id:'first_login',   cat:'Beginner', name:'First Login',    icon:'login',   check:function(s){return s.logins>=1;}},
  {id:'first_post',    cat:'Beginner', name:'First Post',     icon:'post',    check:function(s){return s.postsCount>=1;}},
  {id:'first_story',   cat:'Beginner', name:'First Story',    icon:'camera',  check:function(s){return s.storiesCount>=1;}},
  {id:'first_comment', cat:'Beginner', name:'First Comment',  icon:'comment', check:function(s){return s.commentsMade>=1;}},
  {id:'first_like',    cat:'Beginner', name:'First Like',     icon:'heart',   check:function(s){return s.likesGiven>=1;}},
  {id:'first_friend',  cat:'Beginner', name:'First Friend',   icon:'people',  check:function(s){return s.followingCount>=1;}},
  {id:'complete_profile',cat:'Beginner',name:'Complete Profile',icon:'check', check:function(s){return s.profileComplete;}},
  // Creator
  {id:'posts_10',   cat:'Creator', name:'10 Posts',   icon:'stack', progress:'postsCount',   target:10,   check:function(s){return s.postsCount>=10;}},
  {id:'stories_25', cat:'Creator', name:'25 Stories',  icon:'film', progress:'storiesCount', target:25,   check:function(s){return s.storiesCount>=25;}},
  {id:'likes_100',  cat:'Creator', name:'100 Likes Received',  icon:'heart', progress:'likesReceived', target:100,  check:function(s){return s.likesReceived>=100;}},
  {id:'likes_500',  cat:'Creator', name:'500 Likes Received',  icon:'heart', progress:'likesReceived', target:500,  check:function(s){return s.likesReceived>=500;}},
  {id:'likes_1000', cat:'Creator', name:'Popular Creator (1000 Likes)', icon:'star', progress:'likesReceived', target:1000, check:function(s){return s.likesReceived>=1000;}},
  // Community
  {id:'comments_100', cat:'Community', name:'100 Comments',   icon:'chatMulti', progress:'commentsMade', target:100, check:function(s){return s.commentsMade>=100;}},
  {id:'replies_50',   cat:'Community', name:'50 Replies',     icon:'reply',  progress:'repliesMade',  target:50,  check:function(s){return s.repliesMade>=50;}},
  {id:'reactions_500',cat:'Community', name:'Friendly User (500 Reactions)', icon:'sparkle', progress:'likesGiven', target:500, check:function(s){return s.likesGiven>=500;}},
  // Explorer
  {id:'visit_100', cat:'Explorer', name:'Visit 100 Profiles', icon:'compass', progress:'profilesVisited', target:100, check:function(s){return s.profilesVisited>=100;}},
  {id:'visit_500', cat:'Explorer', name:'Visit 500 Profiles', icon:'map', progress:'profilesVisited', target:500, check:function(s){return s.profilesVisited>=500;}},
  // Streak
  {id:'streak_3',   cat:'Streak', name:'3-Day Streak',   icon:'flame', progress:'loginStreak', target:3,   check:function(s){return s.loginStreak>=3;}},
  {id:'streak_7',   cat:'Streak', name:'7-Day Streak',   icon:'flame', progress:'loginStreak', target:7,   check:function(s){return s.loginStreak>=7;}},
  {id:'streak_30',  cat:'Streak', name:'30-Day Streak',  icon:'flame', progress:'loginStreak', target:30,  check:function(s){return s.loginStreak>=30;}},
  {id:'streak_100', cat:'Streak', name:'100-Day Streak', icon:'flame', progress:'loginStreak', target:100, check:function(s){return s.loginStreak>=100;}},
  {id:'streak_365', cat:'Streak', name:'365-Day Streak', icon:'flame', progress:'loginStreak', target:365, check:function(s){return s.loginStreak>=365;}},
  // Special
  {id:'verified_user', cat:'Special', name:'Verified User', icon:'shield', check:function(s){return !!s.verified;}},
  {id:'founding_member',cat:'Special', name:'Founding Member', icon:'flag', check:function(s){return !!s.foundingMember;}}
];
var _myAchievements = {}; // {achievementId: true} — locally cached, loaded on init

function _computeMyAchievementStats(){
  var myPosts = posts.filter(function(p){return p.uid===me.uid;});
  return {
    logins: me.loginCount||1,
    postsCount: myPosts.filter(function(p){return !p.isStatus&&!p.isPoll;}).length,
    storiesCount: me.storiesPostedCount||0,
    commentsMade: me.commentsMadeCount||0,
    repliesMade: me.repliesMadeCount||0,
    likesGiven: me.likesGivenCount||0,
    likesReceived: myPosts.reduce(function(sum,p){return sum+(p.likes||0);},0),
    followingCount: (me.following||[]).length,
    profilesVisited: me.profilesVisitedCount||0,
    loginStreak: me.loginStreak||0,
    verified: !!me.verified||!!me.isAdmin,
    foundingMember: !!me.foundingMember,
    profileComplete: !!(me.name && me.bio && me.avatar && me.bannerImage)
  };
}

// Call this after any action that could unlock something (post, comment, like, story, visit, follow),
// and once on app init to backfill achievements for existing users who already qualify.
async function checkAchievements(){
  if(!me.uid) return;
  var stats = _computeMyAchievementStats();
  var newlyUnlocked = [];
  ACHIEVEMENT_DEFS.forEach(function(def){
    if(_myAchievements[def.id]) return; // already unlocked
    if(def.check(stats)) newlyUnlocked.push(def);
  });
  if(!newlyUnlocked.length) return;
  for(var i=0;i<newlyUnlocked.length;i++){
    var def = newlyUnlocked[i];
    _myAchievements[def.id] = true;
    try{
      await db.collection('profiles').doc(me.uid).collection('achievements').doc(def.id).set({
        id: def.id, unlockedAt: firebase.firestore.FieldValue.serverTimestamp()
      });
    }catch(e){}
    toast('Achievement unlocked: '+def.name+'!');
    _achievementConfettiBurst();
  }
}
async function loadMyAchievements(){
  if(!me.uid) return;
  try{
    var snap = await db.collection('profiles').doc(me.uid).collection('achievements').get();
    _myAchievements = {};
    snap.docs.forEach(function(d){ _myAchievements[d.id] = true; });
  }catch(e){ _myAchievements = {}; }
  checkAchievements(); // backfill any newly-qualifying achievements, including for existing users on first load
}

// Simple daily login streak — counts a login once per calendar day.
// Note: this is a lightweight version. A stricter "must stay active 10 seconds" rule
// belongs to the full presence-system rework and isn't enforced here yet.
function _updateLoginStreak(){
  var today = new Date().toISOString().slice(0,10); // YYYY-MM-DD
  var last = me.lastLoginDate;
  if(last === today) return; // already counted today
  var streak = me.loginStreak||0;
  if(last){
    var lastMs = new Date(last+'T00:00:00Z').getTime();
    var todayMs = new Date(today+'T00:00:00Z').getTime();
    var diffDays = Math.round((todayMs-lastMs)/86400000);
    streak = diffDays===1 ? streak+1 : 1; // consecutive day → +1, otherwise reset
  } else {
    streak = 1;
  }
  me.loginStreak = streak;
  me.lastLoginDate = today;
  me.loginCount = (me.loginCount||0)+1;
  db.collection('profiles').doc(me.uid).set({
    loginStreak: streak, lastLoginDate: today, loginCount: firebase.firestore.FieldValue.increment(1)
  }, {merge:true}).catch(function(){});
}

// Achievements viewer — locked/unlocked states, progress bars for numeric milestones, grouped by category
async function openAchievementsModal(uid){
  var isMe = !uid || uid===me.uid;
  var unlockedMap = {};
  if(isMe){
    unlockedMap = _myAchievements;
  } else {
    try{
      var snap = await db.collection('profiles').doc(uid).collection('achievements').get();
      snap.docs.forEach(function(d){ unlockedMap[d.id] = true; });
    }catch(e){}
  }
  var stats = isMe ? _computeMyAchievementStats() : null; // progress bars only meaningful for your own profile
  var cats = {};
  ACHIEVEMENT_DEFS.forEach(function(def){
    if(!cats[def.cat]) cats[def.cat] = [];
    cats[def.cat].push(def);
  });
  var pinned = isMe ? (me.pinnedAchievements||[]) : [];
  var html = Object.keys(cats).map(function(cat){
    var items = cats[cat].map(function(def){
      var unlocked = !!unlockedMap[def.id];
      var isPinned = pinned.includes(def.id);
      var progressHTML = '';
      if(!unlocked && def.progress && stats){
        var cur = Math.min(stats[def.progress]||0, def.target);
        var pct = Math.round((cur/def.target)*100);
        progressHTML = '<div style="height:5px;background:var(--bg2);border-radius:3px;overflow:hidden;margin-top:5px;"><div style="height:100%;width:'+pct+'%;background:var(--pink);"></div></div>'
          +'<div style="font-size:10px;color:var(--text3);margin-top:2px;">'+cur+' / '+def.target+'</div>';
      }
      var pinBtn = (isMe && unlocked)
        ? '<button data-ach-id="'+def.id+'" style="font-size:10px;font-weight:700;border:1px solid '+(isPinned?'var(--pink)':'var(--border)')+';color:'+(isPinned?'var(--pink)':'var(--text3)')+';background:none;border-radius:10px;padding:3px 8px;cursor:pointer;flex-shrink:0;">'+(isPinned?'★ Pinned':'☆ Pin')+'</button>'
        : (unlocked?'<div style="font-size:10.5px;color:var(--pink);font-weight:700;flex-shrink:0;">UNLOCKED</div>':'<svg width="16" height="16" fill="none" stroke="var(--text3)" stroke-width="2" viewBox="0 0 24 24" style="flex-shrink:0;"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>');
      return '<div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid var(--border);opacity:'+(unlocked?'1':'.5')+';">'
        +achIconHTML(def.icon, def.cat, 38, unlocked)
        +'<div style="flex:1;min-width:0;"><div style="font-size:12.5px;font-weight:600;">'+esc(def.name)+'</div>'+progressHTML+'</div>'
        +pinBtn
        +'</div>';
    }).join('');
    return '<div style="margin-bottom:6px;"><div style="font-size:11px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.04em;margin:12px 0 2px;">'+cat+'</div>'+items+'</div>';
  }).join('');

  var modal=document.createElement('div');
  modal.id='achievementsModal';
  modal.style.cssText='position:fixed;inset:0;z-index:5300;background:rgba(0,0,0,.6);backdrop-filter:blur(5px);display:flex;align-items:center;justify-content:center;padding:20px;';
  modal.innerHTML='<div style="background:var(--card);border-radius:18px;padding:22px;width:100%;max-width:400px;max-height:82dvh;overflow-y:auto;">'
    +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">'
    +'<div style="font-size:16px;font-weight:700;">🏆 Achievements</div>'
    +'<button onclick="document.getElementById(\'achievementsModal\').remove()" style="background:none;border:none;font-size:20px;cursor:pointer;color:var(--text3);">×</button>'
    +'</div>'
    +(isMe?'<div style="font-size:11.5px;color:var(--text3);">Tap "Profile Info" any time to check your progress toward the next one.</div>':'')
    +html
    +'</div>';
  document.body.appendChild(modal);
  if(isMe){
    modal.querySelectorAll('[data-ach-id]').forEach(function(btn){
      btn.onclick = function(){ togglePinAchievement(btn.dataset.achId); openAchievementsModal(uid); };
    });
  }
}
// Pin/unpin an unlocked achievement to the profile showcase (max 5, matching spec)
function togglePinAchievement(achId){
  if(!Array.isArray(me.pinnedAchievements)) me.pinnedAchievements = [];
  var idx = me.pinnedAchievements.indexOf(achId);
  if(idx>-1){
    me.pinnedAchievements.splice(idx,1);
  } else {
    if(me.pinnedAchievements.length>=5){ toast('You can only pin up to 5 badges — unpin one first'); return; }
    me.pinnedAchievements.push(achId);
  }
  db.collection('profiles').doc(me.uid).set({pinnedAchievements: me.pinnedAchievements},{merge:true}).catch(function(){});
  renderPinnedBadges();
}
// Renders the pinned-badge showcase row on the profile (own profile)
function renderPinnedBadges(){
  var wrap = document.getElementById('myPinnedBadges');
  if(!wrap) return;
  var pinned = me.pinnedAchievements||[];
  if(!pinned.length){ wrap.innerHTML=''; return; }
  wrap.innerHTML = pinned.map(function(id){
    var def = ACHIEVEMENT_DEFS.find(function(d){return d.id===id;});
    if(!def) return '';
    return '<div title="'+esc(def.name)+'">'+achIconHTML(def.icon, def.cat, 34, true)+'</div>';
  }).join('');
...
// ── PROFILE INFORMATION PANEL (v1.2) ──────────────────
async function openProfileInfoPanel(uid){
  var isMe = uid===me.uid;
  var prof = isMe ? me : (allUsers[uid] || {});
  // If we don't have createdAt cached (e.g. viewing someone we haven't fully loaded), fetch it directly
  if(!prof.createdAt){
    try{ var d=await db.collection('profiles').doc(uid).get(); if(d.exists) prof=Object.assign({},prof,d.data()); }catch(e){}
  }
  var theirPosts = posts.filter(function(p){return p.uid===uid;});
  var postsCount = theirPosts.filter(function(p){return !p.isStatus && !p.isPoll;}).length;
  var likesReceived = theirPosts.reduce(function(sum,p){return sum+(p.likes||0);},0);
  var commentsReceived = theirPosts.reduce(function(sum,p){return sum+((p.comments||[]).length);},0);
  var followersEl = document.getElementById(isMe?'statFollowers':'opuStatFollowers');
  var followingEl = document.getElementById(isMe?'statFollowing':'opuStatFollowing');
  var followersN = followersEl ? followersEl.textContent.match(/\d+/) : null;
  var followingN = followingEl ? followingEl.textContent.match(/\d+/) : null;
  var joined = 'Unknown';
  // joinedAt (from Firebase Auth's own creationTime, backfilled at login) is the authoritative source.
  // createdAt is an older Firestore-only field kept as a fallback for accounts that haven't logged in
  // since this was added yet.
  var joinSrc = prof.joinedAt || prof.createdAt;
  if(joinSrc){
    var ms = typeof joinSrc==='number' ? joinSrc : (joinSrc.toMillis ? joinSrc.toMillis() : (joinSrc.seconds ? joinSrc.seconds*1000 : null));
    if(ms) joined = new Date(ms).toLocaleDateString(undefined,{month:'long',day:'numeric',year:'numeric'});
  }
  ...
  function row(label,value){
    return '<div style="display:flex;justify-content:space-between;padding:9px 0;border-bottom:1px solid var(--border);">'
      ...
      +'<span style="font-size:13px;font-weight:600;">'+value+'</span></div>';
  }
  modal.innerHTML='<div style="background:var(--card);border-radius:18px;padding:22px;width:100%;max-width:360px;max-height:80dvh;overflow-y:auto;">'
    +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">'
    +'<div style="font-size:16px;font-weight:700;">Profile Info</div>'
    +'<button onclick="document.getElementById(\'profileInfoModal\').remove()" style="background:none;border:none;font-size:20px;cursor:pointer;color:var(--text3);">×</button>'
    +'</div>'
    +row('Joined',joined)
    +row('Posts',postsCount)
    +row('Likes Received',likesReceived)
    +row('Comments Received',commentsReceived)
    +row('Followers',followersN?followersN[0]:'—')
    +row('Following',followingN?followingN[0]:'—')
    +'<div onclick="openAchievementsModal(\''+esc(uid)+'\')" style="display:flex;justify-content:space-between;padding:9px 0;border-bottom:1px solid var(--border);cursor:pointer;">'
      +'<span style="font-size:13px;color:var(--text3);">Achievements</span>'
      +'<span style="font-size:13px;font-weight:600;color:var(--pink);">View 🏆 ›</span></div>'
    +row('Badges','Coming soon 🎖️')
    +'<div style="font-size:11px;color:var(--text3);margin-top:10px;">Stats reflect posts currently loaded in your feed and may not include your full history yet.</div>'
    +'</div>';
  document.body.appendChild(modal);
}

// ── RECENT PROFILE VISITORS (v1.1) ────────────────────
// Records that I visited someone's profile, keeping only the latest 3 unique visitors.
// Firestore: profiles/{ownerUid}/visitors/{visitorUid} → {visitorId, visitedAt}
async function recordProfileVisit(ownerUid){
  if(!ownerUid || ownerUid===me.uid) return; // don't record visits to your own profile
  try{
    var ref = db.collection('profiles').doc(ownerUid).collection('visitors').doc(me.uid);
    await ref.set({ visitorId: me.uid, visitedAt: firebase.firestore.FieldValue.serverTimestamp() }, {merge:true});
    // Cap at latest 3 unique visitors — remove anything older than the 3 most recent
    var snap = await db.collection('profiles').doc(ownerUid).collection('visitors').orderBy('visitedAt','desc').get();
    if(snap.docs.length > 3){
      var toRemove = snap.docs.slice(3);
      toRemove.forEach(function(d){ d.ref.delete().catch(function(){}); });
    }
    me.profilesVisitedCount = (me.profilesVisitedCount||0)+1;
    db.collection('profiles').doc(me.uid).set({profilesVisitedCount: firebase.firestore.FieldValue.increment(1)},{merge:true}).catch(function(){});
    checkAchievements();
  }catch(e){ /* non-critical — never block the profile view over this */ }
}
// Shows the latest 3 unique visitors, top-right of the profile header, visible to everyone (public feature)
async function loadRecentVisitors(ownerUid, containerId){
  var wrap = document.getElementById(containerId || 'opuRecentVisitors');
  if(!wrap) return;
  wrap.innerHTML='';
  try{
    var snap = await db.collection('profiles').doc(ownerUid).collection('visitors').orderBy('visitedAt','desc').limit(3).get();
    if(!snap.docs.length) return;
    var stack = snap.docs.map(function(d,i){
      var vUid = d.id;
      var prof = allUsers[vUid] || {};
      var av = prof.avatar
        ? '<img src="'+esc(prof.avatar)+'" style="width:100%;height:100%;object-fit:cover;">'
        : '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:'+(prof.color||'linear-gradient(135deg,#e2688a,#f0a0b8)')+';color:white;font-size:11px;font-weight:700;">'+(prof.initial||'?')+'</div>';
      return '<div onclick="event.stopPropagation();viewProfile(\''+esc(vUid)+'\')" title="'+esc(prof.name||'User')+'" style="width:26px;height:26px;border-radius:50%;overflow:hidden;border:2px solid white;margin-left:'+(i>0?'-8px':'0')+';cursor:pointer;position:relative;box-shadow:0 1px 3px rgba(0,0,0,.25);">'+av+(prof.verified?'<div style="position:absolute;bottom:-2px;right:-2px;">'+verifiedBadgeHTML({verified:true},10)+'</div>':'')+'</div>';
    }).join('');
    wrap.innerHTML = '<div style="display:flex;align-items:center;background:rgba(0,0,0,.35);backdrop-filter:blur(4px);border-radius:20px;padding:4px 8px 4px 4px;cursor:default;" title="Recent visitors">'+stack+'</div>';
  }catch(e){ /* non-critical */ }
}

// Mutual friends = people I follow who are also followers of this profile (standard "Followed by" pattern)
async function loadMutualFriends(uid){
  var wrap=document.getElementById('opuMutualFriends');
  if(!wrap || uid===me.uid) return;
  wrap.innerHTML='';
  try{
    var mySnap = await db.collection('profiles').doc(me.uid).collection('following').get();
    var myFollowingUids = mySnap.docs.map(function(d){return d.id;});
    if(!myFollowingUids.length) return;
    var theirFollowersSnap = await db.collection('profiles').doc(uid).collection('followers').get();
    var theirFollowerUids = new Set(theirFollowersSnap.docs.map(function(d){return d.id;}));
    var mutualUids = myFollowingUids.filter(function(u){return theirFollowerUids.has(u) && u!==me.uid && u!==uid;});
    if(!mutualUids.length) return;
    var mutualNames = mutualUids.slice(0,3).map(function(u){
      var prof = allUsers[u];
      var name = prof ? (prof.name||prof.handle||'Someone') : 'Someone';
      return '<span onclick="event.stopPropagation();viewProfile(\''+esc(u)+'\')" tabindex="0" role="button" onkeydown="if(event.key===\'Enter\')viewProfile(\''+esc(u)+'\')" style="color:var(--text2);font-weight:700;cursor:pointer;text-decoration:none;" onmouseover="this.style.textDecoration=\'underline\'" onmouseout="this.style.textDecoration=\'none\'">'+esc(name)+'</span>';
    });
    var extra = mutualUids.length>3 ? ' + '+(mutualUids.length-3)+' more' : '';
    var peopleIcon = '<svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="vertical-align:-1px;margin-right:3px;"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>';
    wrap.innerHTML = '<div style="font-size:12px;color:var(--text3);">'+peopleIcon+'Followed by '+mutualNames.join(', ')+extra+'</div>';
  }catch(e){ /* fail silently — this is a nice-to-have, not critical */ }
}

async function loadOtherProfileHighlights(uid){
  var wrap=document.getElementById('opuHighlights');
  if(!wrap) return;
  wrap.innerHTML='';
  try{
    var snap=await db.collection('profiles').doc(uid).collection('highlights').orderBy('createdAt','desc').get();
    var hls=snap.docs.map(function(d){return {id:d.id,...d.data()};});
    if(!hls.length) return; // no highlights → show nothing, matches Instagram behavior
    var row=document.createElement('div');
    row.style.cssText='display:flex;gap:14px;overflow-x:auto;padding:12px 16px 4px;scrollbar-width:none;-webkit-overflow-scrolling:touch;';
    ...
    wrap.appendChild(row);
  }catch(e){ /* no highlights or permission issue — show nothing, fail silently like Instagram does */ }
}

// ── HIGHLIGHT EDITOR ───────────────────────────────────
// Rename · change cover · remove story · reorder · add more stories · delete
var _editingHighlightId = null;
function openHighlightEditor(hlId){
  var h=_highlights.find(function(x){return x.id===hlId;});
  if(!h) return;
  _editingHighlightId=hlId;
  var modal=document.createElement('div');
  modal.id='hlEditorModal';
  modal.style.cssText='position:fixed;inset:0;z-index:5200;background:rgba(0,0,0,.75);backdrop-filter:blur(6px);display:flex;align-items:flex-end;justify-content:center;';
  document.body.appendChild(modal);
  _renderHighlightEditor(h);
}
function _closeHighlightEditor(){
  var m=document.getElementById('hlEditorModal'); if(m) m.remove();
  _editingHighlightId=null;
}
// Removes the editor's DOM only — keeps _editingHighlightId alive, used when handing off to the
// archive picker in "add more stories" mode so the eventual 📌 tap knows which highlight to target.
function _closeHighlightEditorDomOnly(){
  var m=document.getElementById('hlEditorModal'); if(m) m.remove();
}
function _renderHighlightEditor(h){
  var modal=document.getElementById('hlEditorModal');
  if(!modal) return;
  var stories=h.stories||[];
  var storiesHTML = stories.length ? stories.map(function(s,i){
    var isVid=s.mediaType==='video';
    return '<div style="position:relative;flex-shrink:0;width:78px;">'
      +'<div style="width:78px;height:104px;border-radius:10px;overflow:hidden;background:var(--bg3);">'
      +(isVid?'<video src="'+esc(s.mediaUrl)+'" muted style="width:100%;height:100%;object-fit:cover;"></video>':'<img src="'+esc(s.mediaUrl)+'" style="width:100%;height:100%;object-fit:cover;">')
      +'</div>'
      +'<button onclick="_hlEditorRemoveStory('+i+')" title="Remove" style="position:absolute;top:3px;right:3px;width:20px;height:20px;border-radius:50%;background:rgba(0,0,0,.6);border:none;color:white;font-size:11px;cursor:pointer;">✕</button>'
      +'<button onclick="_hlEditorSetCover('+i+')" title="Set as cover" style="position:absolute;bottom:22px;left:3px;background:rgba(0,0,0,.6);border:none;color:white;font-size:9px;padding:2px 5px;border-radius:5px;cursor:pointer;">Cover</button>'
      +'<div style="display:flex;justify-content:space-between;margin-top:3px;">'
      +'<button onclick="_hlEditorMove('+i+',-1)" '+(i===0?'disabled style="opacity:.3;"':'')+' style="background:none;border:1px solid var(--border);border-radius:6px;width:22px;height:20px;font-size:11px;cursor:pointer;color:var(--text2);">‹</button>'
      +'<button onclick="_hlEditorMove('+i+',1)" '+(i===stories.length-1?'disabled style="opacity:.3;"':'')+' style="background:none;border:1px solid var(--border);border-radius:6px;width:22px;height:20px;font-size:11px;cursor:pointer;color:var(--text2);">›</button>'
      +'</div>'
      +'</div>';
  }).join('') : '<div style="color:var(--text3);font-size:12.5px;padding:12px 2px;">No stories yet — tap "Add Stories" below.</div>';

  modal.innerHTML='<div style="background:var(--card);border-radius:22px 22px 0 0;padding:22px 20px max(90px,env(safe-area-inset-bottom));width:100%;max-width:520px;max-height:85dvh;overflow-y:auto;">'
    +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">'
    +'<div style="font-size:16px;font-weight:700;">Edit Highlight</div>'
    +'<button onclick="_closeHighlightEditor()" style="background:none;border:none;font-size:22px;cursor:pointer;color:var(--text3);">×</button>'
    +'</div>'
    +'<input id="hlEditorTitle" type="text" value="'+esc(h.title||'Highlight')+'" maxlength="20" style="width:100%;background:var(--bg2);border:1.5px solid var(--border);border-radius:12px;padding:10px 14px;color:var(--text);font-family:Jost,sans-serif;font-size:14px;outline:none;box-sizing:border-box;margin-bottom:16px;">'
    +'<div style="font-size:12px;color:var(--text3);margin-bottom:8px;font-weight:600;">STORIES ('+stories.length+')</div>'
    +'<div style="display:flex;gap:10px;overflow-x:auto;padding-bottom:6px;margin-bottom:16px;">'+storiesHTML+'</div>'
    +'<button onclick="_closeHighlightEditorDomOnly();openStoryArchive();_hlArchiveAddMode=true;" style="width:100%;padding:11px;border-radius:12px;border:1.5px solid var(--pink);background:none;color:var(--pink);font-family:Jost,sans-serif;font-size:13.5px;font-weight:600;cursor:pointer;margin-bottom:10px;">+ Add Stories from Archive</button>'
    +'<button onclick="_hlEditorSave()" style="width:100%;padding:12px;border-radius:12px;border:none;background:var(--pink);color:white;font-family:Jost,sans-serif;font-size:14px;font-weight:600;cursor:pointer;margin-bottom:10px;">Save Changes</button>'
    +'<button onclick="deleteHighlight(\''+h.id+'\')" style="width:100%;padding:11px;border-radius:12px;border:none;background:none;color:#e11d48;font-family:Jost,sans-serif;font-size:13.5px;font-weight:600;cursor:pointer;">Delete Highlight</button>'
    +'</div>';
}
function _hlEditorRemoveStory(i){
  var h=_highlights.find(function(x){return x.id===_editingHighlightId;});
  if(!h) return;
  h.stories.splice(i,1);
  _renderHighlightEditor(h);
}
function _hlEditorSetCover(i){
  var h=_highlights.find(function(x){return x.id===_editingHighlightId;});
  if(!h) return;
  h.coverUrl=h.stories[i].mediaUrl;
  toast('Cover updated — tap Save Changes');
}
function _hlEditorMove(i,dir){
  var h=_highlights.find(function(x){return x.id===_editingHighlightId;});
  if(!h) return;
  var j=i+dir;
  if(j<0||j>=h.stories.length) return;
  var tmp=h.stories[i]; h.stories[i]=h.stories[j]; h.stories[j]=tmp;
  _renderHighlightEditor(h);
}
async function _hlEditorSave(){
  var h=_highlights.find(function(x){return x.id===_editingHighlightId;});
  if(!h) return;
  var newTitle=(document.getElementById('hlEditorTitle')||{}).value;
  if(newTitle!=null) h.title=newTitle.trim()||'Highlight';
  try{
    await db.collection('profiles').doc(me.uid).collection('highlights').doc(h.id).update({
      title:h.title, stories:h.stories||[], coverUrl:h.coverUrl||(h.stories[0]&&h.stories[0].mediaUrl)||''
    });
    renderHighlights();
    _closeHighlightEditor();
    toast('Highlight updated! ✨');
  }catch(e){toast('Could not save changes');}
}
// When "+ Add Stories from Archive" is used, redirect the 📌 tap straight into the highlight being edited
var _hlArchiveAddMode=false;

function openHighlightViewer(h){
  if(!h.stories||!h.stories.length){toast('No stories in this highlight yet');return;}
  var modal=document.createElement('div');
  modal.id='hlViewerModal';
  ...
  function render(){
    ...
    modal.innerHTML='<div style="position:relative;width:100%;max-width:380px;height:100dvh;max-height:700px;">'
      ...
      +h.stories.map(function(_,i){
        var innerStyle = i<idx
          ? 'height:100%;background:white;width:100%;'
          : (i===idx && !isVid)
            ? 'height:100%;background:white;width:0%;animation-duration:'+PHOTO_MS+'ms;'
            : 'height:100%;background:white;width:0%;';
        var animCls = (i===idx && !isVid) ? ' class="hl-progress-fill-anim"' : '';
        return '<div style="flex:1;height:3px;background:rgba(255,255,255,.35);border-radius:2px;overflow:hidden;">'
          +'<div'+animCls+' style="'+innerStyle+'"></div>'
          +'</div>';
      }).join('')
      +'</div>'
      +'<button onclick="_closeHighlightViewerModal()" style="position:absolute;top:24px;right:14px;z-index:10;background:rgba(0,0,0,.4);border:none;color:white;font-size:20px;cursor:pointer;border-radius:50%;width:34px;height:34px;display:flex;align-items:center;justify-content:center;">×</button>'
      ...
      +(isVid?'<video id="hlViewerVideo" src="'+esc(s.mediaUrl)+'" style="width:100%;height:100%;object-fit:cover;" autoplay muted playsinline></video>':'<img src="'+esc(s.mediaUrl)+'" style="width:100%;height:100%;object-fit:cover;">')
      +(s.caption?'<div style="position:absolute;bottom:20px;left:14px;right:14px;color:white;font-size:13px;background:rgba(0,0,0,.35);padding:8px 12px;border-radius:10px;">'+esc(s.caption)+'</div>':'')
      +'<button onclick="_hlViewerNav(-1)" style="position:absolute;left:0;top:0;bottom:0;width:40%;background:transparent;border:none;cursor:pointer;z-index:5;"></button>'
      +'<button onclick="_hlViewerNav(1)" style="position:absolute;right:0;top:0;bottom:0;width:40%;background:transparent;border:none;cursor:pointer;z-index:5;"></button>'
      +'</div>';
    // Auto-advance: photos advance after a fixed delay; videos advance when they finish playing (no more infinite loop)
    if(isVid){
      var vidEl=document.getElementById('hlViewerVideo');
      if(vidEl){ vidEl.onended=function(){ next(); }; }
    } else {
      advanceTimer=setTimeout(next, PHOTO_MS);
    }
  }
  function next(){
    if(idx>=h.stories.length-1){ _closeHighlightViewerModal(); return; }
    idx=idx+1; render();
  }
  function prev(){ idx=Math.max(idx-1,0); render(); }
  window._hlViewerNav=function(dir){ if(dir>0) next(); else prev(); };
  window._closeHighlightViewerModal=function(){
    clearAdvanceTimer();
    var m=document.getElementById('hlViewerModal'); if(m) m.remove();
  };
  render();
  document.body.appendChild(modal);
}
...
function deleteHighlight(hlId){
  ...
  renderHighlights();
  if(_editingHighlightId===hlId) _closeHighlightEditor();
...
async function votePoll(postId, optionIdx){
  ...
  // Check expiry — support BOTH field names
  var deadline = p.pollEnds || p.pollEndsAt || 0;
  if(deadline && Date.now() > deadline){ toast('This poll has ended ⏰'); return; }

  if(!p.pollVotes) p.pollVotes = {};
  if(!p.pollOptions || !p.pollOptions.length){ toast('Poll has no options'); return; }

  // Normalise ALL options to {text, votes} objects (handles old string-only format)
  p.pollOptions = p.pollOptions.map(function(o){
    if(typeof o === 'string') return {text: o, votes: 0};
    return {text: o.text||'', votes: Number(o.votes)||0};
  });

  var prevVoteIdx = (p.pollVotes[me.uid] !== undefined && typeof p.pollVotes[me.uid] === 'number')
    ? p.pollVotes[me.uid] : -1;

  // Same option clicked again → no-op
  if(prevVoteIdx === optionIdx){ toast('Already voted for this! ✓'); return; }

  // Undo previous vote if changing
  if(prevVoteIdx >= 0 && p.pollOptions[prevVoteIdx]){
    p.pollOptions[prevVoteIdx].votes = Math.max(0, p.pollOptions[prevVoteIdx].votes - 1);
  }
  ...
  var choiceText = p.pollOptions[optionIdx].text || ('Option '+(optionIdx+1));

  // Optimistically update UI immediately (realtime feel)
  _rerenderPollCard(postId, p);

  try {
    await postsCol().doc(postId).update({
      pollOptions: p.pollOptions,
      pollVotes:   p.pollVotes,
      pollEnds:    deadline || null
    });

    toast(prevVoteIdx >= 0 ? 'Vote changed to "'+choiceText+'"! 🗳️' : 'Vote cast for "'+choiceText+'"! 🗳️');

    // Send notification only on first vote (not on change)
    if(prevVoteIdx < 0 && p.uid && p.uid !== me.uid){
      sendNotification(p.uid, 'vote', me, 'voted for "'+choiceText+'" on your poll', postId);
    }
  } catch(e){
    // Rollback on failure
    p.pollOptions[optionIdx].votes = Math.max(0, p.pollOptions[optionIdx].votes - 1);
    if(prevVoteIdx >= 0 && p.pollOptions[prevVoteIdx]){
      p.pollOptions[prevVoteIdx].votes = (p.pollOptions[prevVoteIdx].votes || 0) + 1;
    }
    if(prevVoteIdx >= 0) p.pollVotes[me.uid] = prevVoteIdx;
    else delete p.pollVotes[me.uid];
    _rerenderPollCard(postId, p);
    toast('Could not save vote — check connection');
    console.error('votePoll error:', e.code || e.message, e);
  }
}

// Rerender just the poll section of a post card without full feed re-render
function _rerenderPollCard(postId, p){
  var card = document.getElementById('pc-'+postId);
  if(!card) return;
  var pollWrap = card.querySelector('[data-poll-wrap]');
  if(pollWrap){
    pollWrap.outerHTML = _buildPollHTML(postId, p);
  } else {
    // fallback — full feed re-render
    setTimeout(function(){ renderFeed(); }, 100);
  }
}

// Build just the poll HTML fragment (used by buildCard + _rerenderPollCard)
function _buildPollHTML(pid, p){
  var deadline = p.pollEnds || p.pollEndsAt || 0;
  var pollEnded = deadline > 0 && Date.now() > deadline;

  // Normalise options
  var opts = (p.pollOptions||[]).map(function(o){
    if(typeof o==='string') return {text:o, votes:0};
    return {text: o.text||'', votes: Number(o.votes)||0};
  });

  var myVoteIdx = (p.pollVotes && p.pollVotes[me.uid] !== undefined && typeof p.pollVotes[me.uid]==='number')
    ? p.pollVotes[me.uid] : -1;

  // For OLD string-keyed format fallback
  if(myVoteIdx < 0 && p.pollVotes){
    opts.forEach(function(o,i){
      if(Array.isArray(p.pollVotes[o.text]) && p.pollVotes[o.text].includes(me.uid)) myVoteIdx = i;
    });
  }

  var totalVotes = opts.reduce(function(s,o){ return s + (Number(o.votes)||0); }, 0);
  // Fallback count from pollVotes keys if opts.votes all zero (migration scenario)
  if(totalVotes === 0 && p.pollVotes){
    totalVotes = Object.values(p.pollVotes).filter(function(v){ return typeof v==='number'; }).length;
  }

  var voted = myVoteIdx >= 0;
  var showResults = voted || pollEnded;

  // Photos row
  var photosHTML = '';
  var photos = [p.pollPhotoA, p.pollPhotoB, p.pollPhotoC].filter(Boolean);
  if(photos.length){
    photosHTML = '<div style="display:grid;grid-template-columns:'+Array(photos.length).fill('1fr').join(' ')+';gap:6px;margin-bottom:12px;">';
    photos.forEach(function(url, pi){
      var isVoted = myVoteIdx === pi;
      photosHTML += '<div style="position:relative;border-radius:10px;overflow:hidden;aspect-ratio:1;border:2.5px solid '+(isVoted?'var(--pink)':'transparent')+';transition:border-color .2s;">'
        +'<img src="'+esc(url)+'" style="width:100%;height:100%;object-fit:cover;display:block;">'
        +'<div style="position:absolute;bottom:0;left:0;right:0;background:linear-gradient(transparent,rgba(0,0,0,.65));padding:8px 6px 5px;display:flex;justify-content:space-between;align-items:flex-end;">'
          +'<span style="color:white;font-size:11px;font-weight:800;background:rgba(0,0,0,.35);padding:1px 6px;border-radius:5px;">'+String.fromCharCode(65+pi)+'</span>'
          +(isVoted?'<span style="color:var(--pink);font-size:12px;font-weight:900;">✓</span>':'')
        +'</div>'
      +'</div>';
    });
    photosHTML += '</div>';
  ...
  var optsHTML = opts.map(function(opt, i){
    var votes = Number(opt.votes)||0;
    var pct = totalVotes > 0 ? Math.round(votes/totalVotes*100) : 0;
    var isMyVote = myVoteIdx === i;

    if(showResults){
      return '<div style="margin-bottom:8px;">'
        +'<div style="display:flex;justify-content:space-between;font-size:12.5px;font-weight:'+(isMyVote?'700':'500')+';color:'+(isMyVote?'var(--pink)':'var(--text)')+';margin-bottom:4px;">'
          +'<span>'+(isMyVote?'✓ ':'')+esc(opt.text)+'</span>'
          +'<span>'+votes+' vote'+(votes!==1?'s':'')+' ('+pct+'%)</span>'
        +'</div>'
        +'<div style="background:var(--bg3);border-radius:8px;height:8px;overflow:hidden;">'
          +'<div style="background:'+(isMyVote?'var(--pink)':'var(--border)')+';height:100%;border-radius:8px;width:'+pct+'%;transition:width .6s ease;"></div>'
        +'</div>'
        // Allow vote change if not ended
        +(!pollEnded?'<div style="text-align:right;margin-top:3px;"><button onclick="votePoll(\''+pid+'\','+i+')" style="background:none;border:none;color:var(--text3);font-size:10.5px;cursor:pointer;font-family:Jost,sans-serif;padding:0;transition:color .15s;" onmouseover="this.style.color=\'var(--pink)\'" onmouseout="this.style.color=\'var(--text3)\'">'+(isMyVote?'voted':'change vote')+'</button></div>':'')
      +'</div>';
    } else {
      return '<button onclick="votePoll(\''+pid+'\','+i+')" style="display:block;width:100%;text-align:left;background:var(--bg2);border:1.5px solid var(--border);border-radius:10px;padding:10px 14px;color:var(--text);font-family:Jost,sans-serif;font-size:13.5px;cursor:pointer;margin-bottom:8px;transition:border-color .2s,background .2s;" onmouseover="this.style.borderColor=\'var(--pink)\';this.style.background=\'var(--pink-pale)\';" onmouseout="this.style.borderColor=\'var(--border)\';this.style.background=\'var(--bg2)\';">'+esc(opt.text)+'</button>';
    }
  }).join('');
  ...
  return '<div data-poll-wrap="'+pid+'" style="padding:12px 14px 4px;">'
    +'<div style="font-size:15px;font-weight:700;color:var(--text);margin-bottom:12px;">📊 '+esc(p.pollQuestion||'')+'</div>'
    +photosHTML
    +optsHTML
    +'<div style="font-size:11.5px;color:var(--text3);margin-top:6px;">'+totalVotes+' vote'+(totalVotes!==1?'s':'')+timeStr+'</div>'
  +'</div>';
}
...
// ══════════════════════════════════════════════════════
// REPLY SYSTEM — showReplyInput + submitReply
// ══════════════════════════════════════════════════════
function showReplyInput(postId, commentIdx){
  var id='reply-input-'+String(postId)+'-'+commentIdx;
  var wrap=document.getElementById(id);
  if(!wrap)return;
  var isHidden=(wrap.style.display==='none'||wrap.style.display==='');
  wrap.style.display=isHidden?'block':'none';
  if(isHidden){
    var inp=document.getElementById('reply-inp-'+String(postId)+'-'+commentIdx);
    if(inp)setTimeout(function(){inp.focus();},50);
  }
}

function submitReply(postId, commentIdx){
  postId=String(postId);
  var inp=document.getElementById('reply-inp-'+postId+'-'+commentIdx);
  if(!inp)return;
  ...
  var reply={
    uid:me.uid,
    user:{handle:me.handle,color:me.color,initial:me.initial,avatar:me.avatar||null},
    text:txt,
    ts:Date.now(),
    likes:0,
    likedBy:[],
  ...
  inp.value='';
  // Hide reply input
  var wrap=document.getElementById('reply-input-'+postId+'-'+commentIdx);
  ...
  var commentEl=document.getElementById('comment-'+postId+'-'+commentIdx);
  if(commentEl){
    commentEl.outerHTML=commentHTML(c,postId,commentIdx);
  }
  // Notify the original commenter (if not yourself)
  if(c.uid && c.uid!==me.uid){
    sendNotification(c.uid,'comment',me,'replied: '+txt.slice(0,50),postId);
  }
  // Also notify post owner if different from commenter
  if(p.uid && p.uid!==me.uid && p.uid!==c.uid){
    sendNotification(p.uid,'comment',me,'replied on your post',postId);
  }
  toast('Reply posted!');
}

// ══════════════════════════════════════════════════════════════
// STORY VIEWER WIRED HELPERS (called by index.html buttons)
// ══════════════════════════════════════════════════════════════
function deleteCurrentStoryFromViewer(){
  var s=_svStories[_svIdx]; if(!s) return;
  deleteStory(s.id);
}

// Show/hide reply bar + delete btn when story slide renders
// Patch renderStorySlide to wire these up
var _origRenderSlide = renderStorySlide;
renderStorySlide = function(){
  _origRenderSlide.apply(this, arguments);
  var s = _svStories[_svIdx];
  // Show delete button only for own stories
  var delBtn = document.getElementById('svDeleteBtn');
  if(delBtn) delBtn.style.display = (s && s.uid === me.uid) ? '' : 'none';
  // Show reply bar only for others' stories
  var replyBar = document.getElementById('svReplyBar');
  if(replyBar) replyBar.style.display = (s && s.uid !== me.uid) ? 'flex' : 'none';
};

// ══════════════════════════════════════════════════════════════
// CLOUDINARY FOLDER STRUCTURE (reference)
// All uploads now go into organized folders:
//   kez-media/posts/{uid}/        ← post images & videos
//   kez-media/stories/{uid}/      ← story media (24h, then archived)
//   kez-media/polls/{uid}/        ← poll option photos
//   kez-media/avatars/{uid}/      ← profile avatars
//   kez-media/banners/{uid}/      ← cover photos
// Deleted post media URLs → Firestore /deletedMedia collection
// Expired story media → Firestore /storyArchive collection
// ══════════════════════════════════════════════════════════════
