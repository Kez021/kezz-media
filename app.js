...
function commentHTML(c, postId, commentIdx){
  ...
  return '<div class="comment" id="comment-'+String(postId)+'-'+commentIdx+'">'
    ...
    +'<div style="flex:1;">'
      ...
      +'</div>'
      // Action row: Like count + Reply (position:relative so the reaction picker positions correctly)
      +'<div style="position:relative;display:inline-block;">'
      +'<div class="c-reactions" id="creact-'+String(postId)+'-'+commentIdx+'" style="display:none;flex-wrap:nowrap;gap:4px;margin-top:6px;margin-left:2px;padding:6px 8px;background:var(--card);border:1px solid var(--border);border-radius:20px;box-shadow:0 4px 16px rgba(0,0,0,.12);position:absolute;bottom:30px;left:0;z-index:9999;white-space:nowrap;">'+reactionBar+'</div>'
      +'</div>'
  ...
  inner.innerHTML=
    ...
    +'<div class="lightbox-info-side">'
      ...
      +'<div class="lb-comment-input">'
        ...
        +'<button class="chat-send c-send" onclick="addLbComment(\''+pid+'\')"><svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg></button>'
      +'</div>'
    +'</div>';
...
function openEditPost(id){
  ...
  // Thumbnail options + URL editing
  var thumbSection = document.getElementById('epmThumbSection');
  var thumbOptions = document.getElementById('epmThumbOptions');
  // Remove any existing URL edit row or note
  var existingNote = document.getElementById('epmMediaNote');
  if(existingNote) existingNote.remove();
  var existingUrlRow = document.getElementById('epmUrlRow');
  if(existingUrlRow) existingUrlRow.remove();

  if(p.isYoutube){
    thumbSection.style.display = 'none';
    // Add URL edit row for YouTube
    var urlRow = document.createElement('div');
    urlRow.id = 'epmUrlRow';
    urlRow.style.cssText = 'margin-bottom:12px;';
    urlRow.innerHTML = '<div style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px;">YouTube URL</div>'
      +'<input id="epmUrlInput" type="url" placeholder="Paste YouTube URL..." value="'+esc('https://www.youtube.com/watch?v='+(p.ytVideoId||''))+'" style="width:100%;background:var(--bg2);border:1.5px solid var(--border);border-radius:10px;padding:9px 12px;color:var(--text);font-family:Jost,sans-serif;font-size:13px;outline:none;box-sizing:border-box;" onfocus="this.style.borderColor=\'var(--pink)\'" onblur="this.style.borderColor=\'var(--border)\'">';
    thumbSection.parentNode.insertBefore(urlRow, thumbSection);
  } else if(p.isVideo){
    thumbSection.style.display = 'none';
    // Add URL edit row for video
    var urlRow = document.createElement('div');
    urlRow.id = 'epmUrlRow';
    urlRow.style.cssText = 'margin-bottom:12px;';
    urlRow.innerHTML = '<div style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px;">Video URL</div>'
      +'<input id="epmUrlInput" type="url" placeholder="Paste video URL..." value="'+esc(p.videoUrl||'')+'" style="width:100%;background:var(--bg2);border:1.5px solid var(--border);border-radius:10px;padding:9px 12px;color:var(--text);font-family:Jost,sans-serif;font-size:13px;outline:none;box-sizing:border-box;" onfocus="this.style.borderColor=\'var(--pink)\'" onblur="this.style.borderColor=\'var(--border)\'">';
    thumbSection.parentNode.insertBefore(urlRow, thumbSection);
  } else if(imgs.length > 0){
    ...
    // Status or no-image post: allow uploading a photo
    thumbSection.style.display = 'block';
    thumbOptions.innerHTML = '';
  }
...
function closeEditPost(){
  ...
  if(note) note.remove();
  var urlRow = document.getElementById('epmUrlRow');
...
async function saveEditPost(){
  ...
  try {
    ...
    // Handle URL changes for YouTube / Video posts
    var urlInp = document.getElementById('epmUrlInput');
    if(urlInp && urlInp.value.trim()){
      var newUrl = urlInp.value.trim();
      if(p.isYoutube){
        // Extract video ID from new URL
        var ytMatch = newUrl.match(/(?:v=|youtu\.be\/)([A-Za-z0-9_-]{11})/);
        if(ytMatch){
          var newYtId = ytMatch[1];
          updates.ytVideoId = newYtId;
          updates.ytThumb = 'https://img.youtube.com/vi/'+newYtId+'/hqdefault.jpg';
          p.ytVideoId = newYtId;
...
function openCreateHighlight(){
  ...
  modal.style.cssText='position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.7);backdrop-filter:blur(6px);display:flex;align-items:flex-end;justify-content:center;';
  modal.innerHTML='<div style="background:var(--card);border-radius:22px 22px 0 0;padding:24px 24px max(24px,env(safe-area-inset-bottom));width:100%;max-width:480px;max-height:80dvh;overflow-y:auto;">'
