...
function renderNotifs(docs){
  ...
  docs.forEach(d=>{
    ...
    div.innerHTML=
      ...
      +'<div class="notif-text"><strong>'+esc(n.fromName||n.fromHandle||'Someone')+'</strong> '+bodyText+'</div>'
...
async function openStoryArchive(){
  ...
  stories.forEach(function(s){
    ...
    // Add "Save to Highlight" overlay button for non-expired stories with media
    if(!expired && (s.mediaUrl || s.type==='text')){
      ...
      hlBtn.onclick=function(e){
        e.stopPropagation();
        addStoryToHighlight({mediaUrl:s.mediaUrl||'',mediaType:s.type==='video'?'video':'image'});
        closeStoryArchive();
    ...
    item.onclick=function(){
      ...
      else{toast('This story has expired');}
    };
    grid.appendChild(item);
  });
}
